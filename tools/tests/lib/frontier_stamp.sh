#!/bin/sh
# frontier_stamp.sh -- the ONE definition of a generated fixture's provenance
# signature.
#
# HISTORY: this script used to also DECIDE generate-or-skip (`needsmint`), and
...
#     run (see the `mint` rule in build/build.ninja);
...
# constant: both sides of the comparison (the `mint` edge and compose.py) shell
...
case "$cmd" in
  ...
  write)
    ...
    [ -f "$mss" ] ||
      { echo "frontier_stamp: no generated artifact '$mss' to bind" >&2; exit 2; }
