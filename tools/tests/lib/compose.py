...
# RUNTIME write gate below -- the same shrink-only ratchet, enforced twice.
...
def write_gate_prologue():
    ...
    gate arms per script as the burn-down list shrinks, and nothing red
    can appear that the static ratchet had not already promised was clean.
...
def inline_libs(script: str, lib: str, field: str, contract: str,
    ...
    legacy write sites (the kill-bit family) stay callable from legs that
    have not converted yet.  The alias is passed only while the burn-down
    list still carries lib entries, so deleting the last lib waiver flips
    every composed script to strict without touching this code.  In an
    ungated compose __OT6_EMU_RAW is nil and the alias is the identity.
