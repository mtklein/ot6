...
def mint_sig(gen, root, extras=()):
    ...
    lib/frontier_stamp.sh (shell) writes it at generation time, this
    ...
    that could drift), we shell out to frontier_stamp.sh's own `sig`
...
def stamp_check(name, root):
    ...
    The generation-time check (the ninja graph + lib/frontier_stamp.sh)
    regenerates a fixture when its generator or any lib half changes -- but
    only when `make frontier` runs.  A fixture can still reach a test WITHOUT
    ...
    if not art:
        return (f"fixture {base} is UNBOUND -- its stamp predates the "
                ...
                f"`make frontier` to regenerate it (issue #75)")
    ...
    if actual != art[0]:
        return (f"fixture {base} is UNBOUND -- build/states/{base}.mss "
                ...
                f"generation run; re-run `make frontier` to regenerate it "
                f"(issue #75)")
    ...
    if anc:
        ...
        if anc_actual != digest_:
            return (f"fixture {base} is STALE -- its ancestor {path} "
                    ...
                    f"it moved; re-run `make frontier` to regenerate it "
                    f"(issue #75)")
    return None
...
def check_states(root):
    ...
    # edges -- see frontier_ninja.py's header), so "what did the last run
...
def selftest() -> int:
    ...
    #    frontier_stamp.sh (pointed at a mock tree via OT6_ROOT exactly the
    #    way mint_sig points it).  The bug class is a fixture that LOOKS
...
def main() -> int:
    ...
    if states:
        ...
        # refreshes only under `make frontier`, which `make test` never runs).
