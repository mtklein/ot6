...
-- bal_mines.lua -- multi-battle auto-battler over the LIVE random-encounter
-- pool of the mines chase map (map 50), from the mines_chase.mss fixture:
-- OCTO (Terra) L5, alone, normal Fight/Magic/Item commands, Mithril Knife
...
--    step from RNG stream $1fa1 and the formation from stream $1fa2. Battle k
--    writes documented seed values before pacing; the seed list below covers
--    every formation slot of the map's encounter group, including the 1/16
--    slot, so it is reproducible by construction.
...
local NBATTLES = 8
-- co-tune poke: if set, write these into the ROM before each battle so the
-- HP-multiplier x resistance grid sweeps without a rebuild. POKE_HP ->
-- Ot6HpMulTbl band0 ($10=1x, $18=1.5x, $20=2x); POKE_SHIELD ->
-- Ot6ShieldedMulW ($08=0.5x, $0c=0.75x, $10=1x/off). nil = shipped bytes.
local POKE_HP = nil
local POKE_SHIELD = nil
-- ROM offsets derive from ff6/rom/ff6-en.dbg at compose time (H.sym).
-- `& 0x3FFFFF` turns the CPU address into the snesPrgRom file offset
-- (bank $F0 -> $30xxxx).
...
-- paced steps). The chosen mix approximates the pool's own 5/16, 5/16, 6/16.
local SEEDS = {
...
-- Fire cast: from the command menu, the magic list opens with the cursor on
...
-- the deliberately poor play: bank to 3, then spend a 3-BP boosted Fight at
-- the default target. Fight is pierce, and every mines-pool species is
-- formula (no class weakness), so this boost always lands on a
-- shielded-unweak target, which is the case where boost is wasted.
...
local function sample()
  ...
  for _, c in ipairs(chars) do
    ...
    elseif b < c.bp then
      ...
      -- classify the default target's state at the moment BP was spent.
      -- Fight matches no pool weakness, so a boosted Fight here is
      -- shielded-unweak.
      local tgt
      for _, m in ipairs(mons) do
...
local function paceStep(k)
  ...
  -- encounter fires through an event script, so eventRunning alone is
  -- normal here: pacing goes hands-off during any event and only voids if
  -- no battle follows within 600 frames. The predicate never raises: void
  -- reasons flow into the report.
...
-- ot6_field's route(); tests keep this copy)
