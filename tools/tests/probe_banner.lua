...
--
-- Exec callbacks at fixed bank-C1 addresses sample the PPU scanline/dot
-- via emu.getState() at four points in every battle NMI:
...
-- ("ppu.scanline"), not nested tables.  cpu.cycleCount supplies
-- sub-scanline deltas.
local function ppu()
...
H.run({ maxFrames = 12000 }, {
  ...
  H.call(function()
    ...
    for _, r in ipairs(rec) do
      -- scanlines < 100 mean we wrapped past vblank into the next frame's
      -- active display (spill); express as 262 + sl.
      local fe = r.fe or -1
