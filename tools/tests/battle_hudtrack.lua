...
--
-- Ot6BgHudLine's anchor word (OT6_SHADOW+0 per line) recomputes every frame,
-- holds while a battle animation script is executing (OT6_SCRIPTBUSY
-- brackets BtlGfx_04), and writes only on a real change.
--
-- rapids_start boots into the Lete River's forced fight, whose entrance
-- slides the shown monsters across the field (battle_hudtrail covers the
-- abandoned-cell blanking of this same slide; this file covers the
-- anchors).
--
...
--      stores per live line), the anchors moved, and the writes then stop
--      (a settle point of 90 consecutive quiet frames is found).
--   2. static: 600 more idle frames with no anchor writes on any line, the
--      jitter non-regression on the live lines and the compare-before-store
--      discipline on the empty ones.
--   3. animation: a no-damage action, Banon's own Health command, picked
--      through the live $7BC2 menu, resolves while the enemy side acts
--      freely.  Positive controls: Banon's bp regens +1 (Ot6ActionEnd, so
--      the action resolved) and OT6_SCRIPTBUSY was observed up, so
--      animation scripts ran.  Still no anchor writes: transients are held
--      rather than chased.
--
...
-- the ride into the forced fight: a dialog-paced drive
