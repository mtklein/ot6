-- probe_fc_dump394.lua -- dump 394's tile props with every stair reveal
-- applied (fc_alcove2's state) for the offline Atma-approach solve.
local H = dofile("tools/tests/lib/ot6.lua")
local function mapIs(m) return (H.mapId() & 0x3ff) == m end
H.run({ maxFrames = 20000 }, {
  H.loadState("build/states/fc_alcove2.mss.lua"),
  H.waitFrames(60),
  H.navTo(8, 8, { maxFrames = 4000,
    arrive = function() return mapIs(394) end }),
  H.driveUntil(function() return mapIs(394) end, 900,
    { H.call(function() H.setPad({ up = true }) end) }, "to 394"),
  H.waitFrames(60),
  H.call(function()
