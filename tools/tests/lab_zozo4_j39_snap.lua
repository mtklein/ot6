-- @manual
-- lab_zozo4_j39_snap.lua -- DIAGNOSTIC copy of gen_zozo4_dadaluma.lua that
-- emits savestates inside the J39-row battle (at its first frame and at the
-- first target-select $38 park) without adding a step or a frame, so the
-- potion-route dadaluma_entry attempt-1 hang can be branched (#176).
-- Otherwise byte-for-byte the generator.
...
local function encounters(what, onWipe)
  ...
  return function()
    ...
    if H.battleLoadStarted() then
      LAB = LAB or {}
      if what == "J39 row westbound" then
        if not LAB.r0 then LAB.r0 = H.requestSaveState() end
        if LAB.r0.done and not LAB.e0 then LAB.e0 = true; H.emitBlob("_lab_j39_start.mss", LAB.r0.blob) end
        LAB.n38 = (H.readByte(0x7BC2) == 0x38) and (LAB.n38 or 0) + 1 or 0
        if LAB.n38 == 30 and not LAB.r1 then LAB.r1 = H.requestSaveState() end
        if LAB.r1 and LAB.r1.done and not LAB.e1 then LAB.e1 = true; H.emitBlob("_lab_j39_st38.mss", LAB.r1.blob) end
      end
      F.frame()
      return true
    end
