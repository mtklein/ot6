-- gen_two_done.lua -- the scenario STACK's acceptance test: boot the
-- stacked Terra ending (t2_terra_done, generated by the Terra chain replayed
-- on top of locke_done via OT6_STACK=t2_) and prove ONE playthrough now
-- carries BOTH completions.  Generates:
--   two_done.mss  map 9 (8,3), SCENARIO_MOG alone, controllable, with
--                 $001E AND $0021 both set -- the state no single
--                 input-driven chain could reach, and the canonical boot
--                 for anything that wants "two scenarios down, Sabin's to
--                 go".
--
...
-- class compose.py's selftest guards), the input-driven chain would still
-- generate a t2_terra_done and only these asserts would catch that its
-- flags are wrong.  The generate itself is a re-save of the same
