-- gen_two_done.lua -- acceptance test for the scenario stack: boot the stacked
-- Terra ending (t2_terra_done, generated by the Terra chain replayed on top of
-- locke_done via OT6_STACK=t2_) and check that one playthrough carries both
-- completions.  Generates:
--   two_done.mss  map 9 (8,3), SCENARIO_MOG alone, controllable, with both
...
-- rather than leaving it implied by construction.  If the stack replayed the
-- wrong boot (the failure class compose.py's selftest guards), the
-- input-driven chain would still generate a t2_terra_done, and only these
...
--    SABIN's remains ($032A=1), so the hub is down to one choice.
...
H.run({ maxFrames = 2000 }, {
  ...
  H.logStep(function()
    return string.format("two_done generated at frame %d -- two scenarios in one playthrough", H.frame)
  end),
})
