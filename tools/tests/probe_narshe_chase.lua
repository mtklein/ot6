-- probe_narshe_chase.lua -- runs the chase to $023C, banks it, scans exits.
--
...
H.run({ maxFrames = 80000 }, flatten({
  ...
  H.saveState("wob_chase23C.mss"),
  H.call(function()
    for _, t in ipairs({ {24,10},{31,10},{30,20},{31,20},{37,25},{23,52},
                         {24,11},{31,11},{25,10},{32,10},{23,10} }) do
      local pth = H.bfsPath(t[1], t[2])
      H.log(string.format("  exits: (%d,%d) %s", t[1], t[2],
        pth and (#pth .. " steps") or "NO PATH"))
    end
  end),
  H.logStep(function() return "chase banked at $023C" end),
}))
