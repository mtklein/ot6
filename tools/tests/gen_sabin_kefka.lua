...
local CH = H.newChoice(CHOICES, { tag = "kefka", inBattle = inBattle })
...
local function makeFightPlan(actor)
  ...
  if mx > 0 and hp > 0 and hp * 10 < mx * 6 and itemRow then
    ...
    if id then
      H.log(string.format("kefka: heal f%d e%d %s (hp %d/%d) [%s]",
...
local fightWatch = newDeathWatch("kefka")
...
local function fightPulse(_)
  ...
  if H.frame - fHb >= 300 then
    ...
    H.log(string.format("kefka: fmenu f%d st=%02X actor=%d row=%d itm=%d " ..
  ...
  if ph == 0 then
    if fPlan ~= nil then
      fHeld = fHeld + 1
      if fHeld > 40 then
        H.log(string.format("kefka: plan stalled 40 pulses (st=%02X) -- " ..
...
local function pursuitAttempt(n)
  ...
  return H.cond(function() return not pursuitWon end, {
    ...
    (function()
      ...
      end, "CYAN back at DOMA (map 121), attempt " .. n, 40000)
    end)(),
    H.release(),
    H.waitFrames(30),
    H.call(function()
