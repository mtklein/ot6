...
local COST = { [PUMMEL] = 4, [SUPLEX] = 13, [AIRBLADE] = 28, [BUMRUSH] = 99 }
-- Learn a set spanning 1- AND 2-digit costs so the screenshot shows the cost
-- column's real fit: Pummel(4) Suplex(13) AirBlade(28) Spiraler(50) BumRush(99)
-- -- 99 is the #57 anchor AND the widest a 2-digit cell can ever be.
...
H.run({ maxFrames = 40000 }, {
  ...
  H.call(function()
    ...
    -- 2-digit Bum Rush 99 -- the worst-case width the screenshot must fit).
    H.assertEq(qty[0], COST[PUMMEL],  "Pummel's stamped cost is 4")
    H.assertEq(qty[1], COST[SUPLEX],  "Suplex's stamped cost is 13 (2-digit)")
    H.assertEq(qty[4], COST[BUMRUSH], "Bum Rush's stamped cost is 99 (2-digit, the #57 anchor)")
  end),
