#!/bin/sh
# thamlab_revive_batch.sh -- revive-targeting agent's batch wrapper.
#
#   tools/tests/thamlab_revive_batch.sh <strategy> <seed> [seed ...]
#
#   strategy  ambush-revive | ambush-revive-fix
#             (see probe_thamlab_revive.lua)
#   seed      frames to stand still before engaging (only seed mod 60
#             matters -- $021e has period 60)
#
# Substitutes into probe_thamlab_revive.lua, runs under run.sh with
...
STRATEGY="${1:?usage: thamlab_revive_batch.sh <strategy> <seed> [seed ...]}"
shift 1
