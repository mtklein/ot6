-- gen_reunion_ready.lua -- acceptance test for the full stack, the same shape
-- as gen_two_done one layer up: boot the end of the t3_ chain
-- (t3_reunion_ready, generated by gen_terra_done's reunion fork on the
-- all-three boot) and check that one playthrough carries all three scenario
-- completions plus the reunion.  Generates:
...
-- return opened the if_all at event_main.asm:26654 and rode _caadb9's reunion
-- ("The three have reached Narshe...") to the map-22 staging instead of the
-- hub.  This file asserts the combined claim rather than leaving it implied by
-- construction; the generated state is a re-save of the same controllable
-- moment under the canonical name, at ~0 replay cost.
