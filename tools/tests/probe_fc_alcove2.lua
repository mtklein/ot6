-- probe_fc_alcove2.lua -- crosses the Floating Continent (map 394) from
...
-- trigger, let it reveal, re-scan.  AVOIDED: (70,29) (the "return to the
-- airship?" choice -- the Shadow-posing detour) and (60,11) (the
-- AtmaWeapon approach).  Fights: tactical (the 177-188 pool permits
-- pincers; Behemoth/Dragon are real fights).  Saves fc_alcove2.mss on 358.
...
local function deckPhase(r)
  ...
  return H.cond(function() return mapIs(6) end, {
...
H.run({ maxFrames = 200000 }, flatten({
  ...
  H.call(function()
    H.assertEq((H.readByte(0x1850 + 3) & 0x07) ~= 0, true, "SHADOW in party")
    H.assertEq((H.mapId() & 0x3ff) == 358, true, "reached the save alcove (map 358)")
    H.screenshot("fc_alcove2")
  end),
  H.navTo(8, 10, { maxFrames = 4000 }),
  H.saveState("fc_alcove2.mss"),
