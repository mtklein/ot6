...
local function seqFor(id, tier, slot)
  ...
  -- goes through it.
  -- EDGAR's Tools row is the one costed verb on this raft; BANON's Health
  -- (cmd $1A) falls out of Ot6AbilityCost's chain at vanilla's own cost, 0,
  -- so a boosted Health is free.  Which TOOL the row names is the bag's
  -- business, not the kit's, so H.namedTool reads it the way the ROM builds
  -- the window: bag order, tools flag $40, this actor's own cursor cell.
  local costed = nil
...
local function rideUntil(pred, what, budget, tier)
  ...
  local watch = newDeathWatch("rapids")
    ...
    H.call(function()
      ...
      if (H.gameOverFired or 0) > 0 and lost == nil then
        lost = string.format("GAME OVER counted by the canary in battle #%d " ..
          "at f%d (tier %d) -- party [%s]", #fights, H.frame, tier, partyLine())
        H.log("rapids: " .. lost)
      end
      if wipeN >= 90 and lost == nil then
        lost = string.format("PARTY WIPED in battle #%d at f%d (tier %d) " ..
          "-- party [%s]", #fights, H.frame, tier, partyLine())
        H.log("rapids: " .. lost)
...
local function ride2Attempt(n)
  ...
  return H.cond(function() return not rideWon end, {
    ...
    H.call(function() lost = nil; H.gameOverFired = 0 end),
    (function()
