-- probe_rafterlab_navdiag.lua -- rafter-crossing lab, DIAGNOSTIC copy.
--
-- Identical decision logic to the template's fightline (so a hold=0 run
-- replays build/rafterlab/fightline_h0.log bit-for-bit), plus read-only
-- logging every time bfsPath(14,7) comes back nil: party z ($00b2),
-- reachability of waypoints along the west route, and the object-map
-- occupancy bits for the corridor tiles.  Diagnoses the (19,11)<->(19,12)
-- pacing stall after fight 3.
--
...
STRATEGY = "fightline"                  -- diagnostic copy: fightline only
...
local function cross(res)
  ...
  -- waypoints along the west route (19,12)->(14,7), in walking order
  local WPTS = { {18,12},{16,12},{14,12},{13,12},{13,11},{13,10},{13,9},
                 {12,9},{11,9},{11,8},{11,7},{12,7},{13,7},{14,7} }
  -- corridor tiles whose object-map occupancy matters
  local CORR = { {18,12},{17,12},{16,12},{15,12},{14,12},{13,12},
                 {13,11},{13,10},{13,9},{12,9},{11,9},{11,8},
                 {11,7},{11,6},{11,5},{12,7},{13,7},{14,7} }
  local diagLines, lastDiag, lastChase = 0, -999, -999
  local function diag(x, y, why)
    if diagLines >= 400 then return end
    if hb - lastDiag < 30 then return end
    lastDiag = hb; diagLines = diagLines + 1
    local reach = {}
    for _, w in ipairs(WPTS) do
      local p = H.bfsPath(w[1], w[2])
      reach[#reach+1] = string.format("(%d,%d)%s", w[1], w[2],
        p and "=" .. #p or "*")
    end
    local occ = {}
    for _, t in ipairs(CORR) do
      local b = H.readByte(0x7E2000 + ((t[2] & 0xFF) << 8) + (t[1] & 0xFF))
      if (b & 0x80) == 0 then
        occ[#occ+1] = string.format("(%d,%d)=%02x", t[1], t[2], b)
      end
    end
    H.log(string.format("diag f%d hb%d %s at (%d,%d) b2=%02x timer=%d | reach %s | occ %s | rats %s",
      H.frame, hb, why, x, y, H.readByte(0x00b2), H.readWord(0x1189),
      table.concat(reach, " "), #occ > 0 and table.concat(occ, " ") or "none",
      ratLine()))
  end
  ...
  local function fightlineStep(x, y)
    ...
    if plan and hb - lastDiag >= 300 and diagLines < 400 then
      lastDiag = hb; diagLines = diagLines + 1
      H.log(string.format("diag f%d hb%d plan-ok at (%d,%d) b2=%02x len=%d first=%s,%s,%s",
        H.frame, hb, x, y, H.readByte(0x00b2), #plan,
        tostring(plan[1]), tostring(plan[2]), tostring(plan[3])))
    end
    if not plan then diag(x, y, "bfs-nil") end
    ...
    local bestPlan, bestTgt
    for _, r in ipairs(rats) do
      for _, d in ipairs({ {0,-1}, {1,0}, {0,1}, {-1,0} }) do
        local pl = H.bfsPath(r.x + d[1], r.y + d[2])
        if pl and (not bestPlan or #pl < #bestPlan) then
          bestPlan, bestTgt = pl, { r.x + d[1], r.y + d[2], r.k }
        end
      end
    end
    if bestPlan and hb - lastChase >= 30 and diagLines < 400 then
      lastChase = hb; diagLines = diagLines + 1
      H.log(string.format("diag f%d hb%d chase rat%d-adj (%d,%d) len=%d first=%s",
        H.frame, hb, bestTgt[3], bestTgt[1], bestTgt[2], #bestPlan,
        tostring(bestPlan[1])))
    end
