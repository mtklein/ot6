-- probe_banquet_greedy.lua -- score-tier measurement.
--
...
--
-- The driver is greedy rather than following a fixed route: map 250's
...
-- if this driver clears a tier, the step can ship that tier.
--
...
-- The 243 door row (22..24,34) is a 3-tile long entrance inside an
-- ordinary walkable stretch of 250's corridor.  The door back is closed,
...
-- Right after a map transition, fieldX/fieldY still read the old map's
-- tile for a frame or two, so a bfsPath taken then finds a path navTo
-- cannot follow.  Every pick waits for a settled field state first.
...
local function leastUsedCrossing()
  ...
  for _, c in ipairs(CROSS[map()] or {}) do
    ...
    -- written as an explicit if: `(cond and nil) or bfs(...)` looks like
    -- a guard but is not one, since `X and nil` is nil (falsy), so the
    -- `or` branch always runs.
    local p
...
H.run({ maxFrames = 40000 }, {
  ...
  -- leave the throne tower first: control returns inside it, and its
  -- only circuit-side exit is the (53,35) long entrance
