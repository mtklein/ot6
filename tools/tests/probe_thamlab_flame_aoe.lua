-- probe_thamlab_flame_aoe.lua -- Thamasa fire lab, FlameEater-strategy
-- agent's copy of probe_thamlab_template.lua.  Adds two strategies:
--
--   flame-aoe  the owner's directive made concrete: an ambush-driver-style
--              per-turn plan (spliced from the gen's newAmbushPlan below)
--              where TERRA/LOCKE cast boosted Ice (folds to Ice2, AoE) and
--              STRAGO casts Aqua Rake, steered POSITIONALLY (want row =
--              loreId, the measured shape of the battle lore window; NOT
--              the compacted-count model newAmbushPlan uses).
--   libnuke    the lib newFightDriver with opts.nuke={Ice} and
--              opts.nukeLore={Aqua Rake} (the driver agent's new lines).
--
-- Also adds a HEALER token ("terra" default | "all" = opts.healer nil,
-- anyone may heal) on the control/libnuke driver.
--
-- Otherwise identical to the template; original header follows.
--
...
if STRATEGY:find("@") then STRATEGY = "flame-aoe" end
if HEALER:find("@") then HEALER = "terra" end
...
-- ------------------------------------------------------------------------
-- flame-aoe: the ambush plan's skeleton pointed at FlameEater.  TERRA and
-- LOCKE cast boosted Ice (the boost folds it to Ice2, AoE; the board is
-- weak to ice/water per docs/design/bosses-wob.md and absorbs fire);
-- STRAGO casts Aqua Rake every offensive turn.  Differences from
-- newAmbushPlan, each deliberate:
--   * lore steering is POSITIONAL: want row = loreId (measured today from
--     a state dump; the compacted loreRowForA model is the known-wrong
--     assumption behind the 61k-frame ambush stall).  Cursor = scroll
--     ($891f) + in-window row ($8927), the lib's own pair.
--   * the revive window opens whenever anyone is down (the ambush's
--     "at most one Balloon left" gate never opens with the 8400 HP boss
--     on the board), STRAGO first.
--   * no Filch line: shields here answer to the Ice/Aqua Rake elements,
--     and every LOCKE turn spent stealing is a round of 324-447 party
--     damage uncontested.

local LSCROLL_A = 0x891F

-- Where a lore sits in the actor's live battle Lore window: the window
-- draws the per-actor spell list's lore segment (records 55..78) in record
-- order, so the row is wherever ability id $8B+loreId actually sits there
-- (the lib's loreCell finding, ot6.lua; the compacted-count model is the
-- known-wrong one).  Falls back to row = loreId (the positional shape
-- measured from today's state dump) when the list pointer isn't live yet.
local function loreRowLiveA(actor, loreId)
  local base = H.readWord(MLISTPTR_A + actor * 2)
  if base >= 0x2000 and base <= 0x2600 then
    for row = 0, 23 do
      if H.readByte(base + (55 + row) * 4) == 0x8B + loreId then return row end
    end
  end
  return loreId
end

local function newFlamePlan(tag)
  local F = {}
  local phase, mf = 0, 0
  local turnActor, turnPlan = nil, nil
  local aqCasts, fightBursts, iceCasts = 0, 0, 0
  local stallN = 0                     -- frames since the last landed
                                       -- confirm / fresh plan, MENU open
  local function partySlots()
    local stragoSlot, downSlots = nil, {}
    for e = 0, 3 do
      if H.readWord(BCMAXHP_A + e * 2) > 0 then
        local cid = H.readByte(BCHID_A + e * 2)
        if cid == STRAGO then stragoSlot = e end
        if H.readWord(BCHP_A + e * 2) == 0 then
          downSlots[#downSlots + 1] = e
        end
      end
    end
    return stragoSlot, downSlots
  end
  local function decideTurn(actor)
    local charId = H.readByte(BCHID_A + actor * 2)
    local hp, mx = H.readWord(BCHP_A + actor * 2), H.readWord(BCMAXHP_A + actor * 2)
    local stragoSlot, downSlots = partySlots()
    -- 1. self-heal, always allowed (the fence lesson).  Potion first and
    -- alone: bagIdxOfA scans BAG order, so listing both ids hands the turn
    -- to whichever sits earlier in the bag -- and a 50 HP Tonic against
    -- 324-447/round hits is a wasted turn.  Tonic only once Potions are gone.
    if mx > 0 and hp > 0 and hp < mx * 0.40 then
      if bagIdxOfA({ POTION }) then
        return { kind = "item", ids = { POTION }, target = actor }
      end
      if bagIdxOfA({ TONIC }) then
        return { kind = "item", ids = { TONIC }, target = actor }
      end
    end
    -- 2. revive: anyone down, STRAGO first (his Aqua Rake is the engine)
    if #downSlots > 0 then
      local idx = bagIdxOfA({ FENIX_DOWN })
      if idx then
        local tgt = downSlots[1]
    ...
    -- 3. offense: STRAGO Aqua Rake; TERRA/LOCKE boosted Ice; else Fight
    if charId == STRAGO then
      return { kind = "lore", loreId = AQUA_RAKE_LORE_ID }
    end
    if (charId == TERRA or charId == LOCKE)
       and spellCellA(actor, ICE_SPELL, true) then
  ...
  local function buttonFor(actor, st)
    ...
    if plan.kind == "lore" then
      ...
      elseif st == ST_LORE_OPEN_A then
        return nil                       -- transitional DMA fill, just wait
      elseif st == ST_LORE_A then
        -- engine-read row (see loreRowLiveA above), NOT the compacted count
        local want = loreRowLiveA(actor, plan.loreId)
        local cur = H.readByte(LSCROLL_A + actor) + H.readByte(LROW_A + actor)
        if cur == want then return "a" end
        return cur < want and "down" or "up"
      elseif st == ST_TGT_A then
        return "a"                       -- multi-target, no steer
    ...
    if plan.kind == "magic" then
      -- boost-bank shape shared with the fight fallback: spend BP (capped
      -- at 3) only once at least 2 is banked; OT6 folds the boosted base
      -- cast to its next tier (Ice -> Ice2, AoE) via Ot6FoldTbl.
      if st == ST_CMD_A then
        if not plan.boosted then
          local bp = H.readByte(BP_A + actor * 2)
          local want = (bp >= 2) and math.min(bp, 3) or 0
  ...
  function F.frame()
    ...
    -- drop the plan for an unboosted Fight, which every state can escape.
    stallN = stallN + 1
    if stallN >= 1200 then
      H.log(string.format(
        "[%s] STALL watchdog f%d: actor=%d st=$%02X plan=%s -- forcing a " ..
        "plain Fight", tag, H.frame, actor, st,
        turnPlan and turnPlan.kind or "nil"))
      turnActor, turnPlan = actor, { kind = "fight" }
      stallN = 0
    end
    if (turnPlan == nil or turnActor ~= actor) and st ~= ST_CMD_A then
      H.setPad({})
    ...
    -- lore-stall watchdog, kept from the template: if the POSITIONAL model
    -- is also wrong, dump the evidence once and fall back to Fight so the
    -- run terminates instead of A-tapping 61k frames.
    if turnPlan.kind == "lore" and st == ST_LORE_A then
      turnPlan.loreStall = (turnPlan.loreStall or 0) + 1
      if turnPlan.loreStall == 600 then
        local t306a = {}
        ...
        H.log(string.format(
          "[%s] LORE-STALL dump f%d: actor=%d loreId=%d 3A87=$%02X " ..
          "306A+0..23=[%s] scroll=%d row=%d want(live)=%d",
          tag, H.frame, actor, turnPlan.loreId, H.readByte(0x3A87),
          table.concat(t306a, " "),
          H.readByte(LSCROLL_A + actor), H.readByte(LROW_A + actor),
          loreRowLiveA(actor, turnPlan.loreId)))
        H.log(string.format(
          "[%s] LORE-STALL: abandoning the lore plan for a plain boosted " ..
          "Fight so this run terminates", tag))
        turnPlan = { kind = "fight", boost = true }
    ...
    if st == ST_TGT_A and btn == "a" then
      ...
      elseif turnPlan.kind == "magic" and not turnPlan.counted then
        ...
        stallN = 0
        H.log(string.format("[%s] Ice cast #%d confirmed f%d", tag, iceCasts, H.frame))
      elseif turnPlan.kind == "fight" and not turnPlan.counted then
        turnPlan.counted = true; fightBursts = fightBursts + 1
        stallN = 0
        H.log(string.format("[%s] Fight #%d confirmed f%d", tag, fightBursts, H.frame))
      end
      if turnPlan.kind == "lore" and turnPlan.counted then stallN = 0 end
      if turnPlan.kind == "item" then stallN = 0 end
    end
    H.setPad(btn and { [btn] = true } or {})
  end
  function F.idle()
    turnActor, turnPlan = nil, nil
    H.log(string.format("[%s] tally: Aqua Rake x%d, Ice x%d, Fight x%d",
      tag, aqCasts, iceCasts, fightBursts))
  end
  return F
end
...
local function newStrategyDriver()
  local healer = HEALER == "all" and nil or TERRA
  if STRATEGY == "taps" then return newTapsDriver() end
  if STRATEGY == "flame-aoe" then return newFlamePlan("thamlab-flame-aoe") end
  if STRATEGY == "libnuke" then
    return H.newFightDriver("thamlab-flame-libnuke", {
      tactical = true, boost = BOOST == 1, bank = 3, items = ITEMS == 1,
      cure = CURE == 1, healer = healer, healPercent = HEALPCT,
      nuke = { ICE_SPELL }, nukeLore = { AQUA_RAKE_LORE_ID } })
  end
