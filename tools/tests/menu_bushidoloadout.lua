...
-- menu_bushidoloadout.lua -- the field Skills->SwdTech configurator, run
-- against a real Cyan from the cyan_defence fixture, with no state writes.
-- CYAN's own record carries BUSHIDO; the learned set $1cf7 and the loadout
-- word $1e1d are read, not written.
--
-- Storage is a 16-bit little-endian word at $1e1d..$1e1e: slot s occupies
-- bits s*3..s*3+2, and word 0 = AUTO.  The page has three rows (1x/2x/3x),
-- and menu row i edits word slot i+1; word slot 0 is retired and mirrors
-- slot 1 when written.
--
-- Seeding from AUTO is implicit: entering while AUTO writes nothing (the word
-- stays 0), and each drawn row computes its auto tech on the fly through
-- Ot6LoadoutSlotTech.  Assign: the first edit (R shoulder) freezes the whole
-- auto window into the word, so the untouched slots keep their auto techs,
-- then cycles the cursored slot.  The window, per Ot6LoadoutAutoTech: ceiling
-- c = highest learned tech, base b = max(0, c-2), slot s (1..3) shows
-- min(c, b+s-1).  Revert: Y writes $0000 (AUTO); the display recomputes.
...
H.run({ maxFrames = 20000 }, {
  ...
  -- $59 is the field's own menu-opening flag, readable while the field still
  -- owns the zero page; ZMENUSTATE only means anything once the menu module
  -- has taken over.
  ...
  H.waitUntil(function() return H.readByte(ZMENUSTATE) == ST_MAIN end,
    400, "main menu", 5),
  H.waitFrames(30),
  -- zCharID is only valid while the menu module owns the zero page.
  H.call(function()
    local ids = {}
    for s = 0, 3 do
  ...
  -- row 2 = SwdTech (Genju, Magic, SwdTech, ...)
  H.driveUntil(function()
  ...
  -- The exact step count per press depends on the menu's own key repeat, so
  -- this samples the cursor over a walk instead of pinning one press to one
  -- row.
  H.call(function()
    _G.__maxrow = 0
  ...
  -- R cycles the cursored row 0 (word slot 1) to the next learned tech.
  H.pressButtons({ "r" }, 3),
  H.waitFrames(20),
  H.call(function()
