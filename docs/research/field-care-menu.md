...
### `$00`/`$01`/`$02` are a three-state fade, not one state
...
So every "fade" transition is: 1 frame in `$00`/`$01`, 8 frames in `$02`, then
the target. A poller that waits for an exact state must not be surprised by
`$02`, and **must not treat `$02` as "still in the previous screen"**.
...
- `ScrollListPage` first; **if it returns carry set the whole rest of the state
  is skipped for that frame** (`field_menu.asm:810-811`, routine at
  `field_menu.asm:857-916`, entry `::ScrollListPage:` at `field_menu.asm:879`). Carry is set when L or R paged the list, and also
  when the page is already at an end. This is a real trap: an L/R press eats
  that frame's A/B.
- B → saves cursor state to `w022f`/`w0231`, then `GotoItemOption`
  (`field_menu.asm:815-836`): `zMenuState = $17` **directly, no fade**.
...
(`menu_common.asm:1230-1246`) — with one column **`$4B` is exactly the
inventory slot index**. `$4A` is the page scroll, `$4E` the row within the page,
`$50 = $4A + $4E`.
...
columns, **x wraps** (bit 7 clear). Single page, so
...
- **USE** (`SelectItemOption_00`, `field_menu.asm:2127-2167`) restores the item
  cursor and sets `zMenuState = $08`. **It sets no "use mode" flag** — it is
  literally just "go back to the list". Using an item does not require going
  through USE at all.
- **ARRANGE** (`field_menu.asm:2169-2179`) sorts the inventory by item icon
  ...
  `field_menu.asm:2241-2246`) — this **rewrites `$1869`/`$1969`** — and returns
  to `$17`.
...
### 2.4 Item move `$19` — where "use" actually happens
...
- **A is checked before B** (`item.asm:2149-2157`). If you press both in one
  frame, both fire.
- A → `GetInventoryItemID` (`item.asm:2390-2395`: `$1869[zSelIndex]`); Rename
  ...
  - `CheckCanUseItem` (`item.asm:2243-2330`). **Carry clear = refused.**
  - Refused → `PlayInvalidSfx` + `CreateMosaicTask` and **stay in `$70`**
    (`item.asm:2199-2201`).
  - Accepted → `PlayCureSfx`, `_c38b11` (apply effect + `DecItemQty`,
    `item.asm:2206-2217`), redraw quantity and char blocks, then: if
    `$1969[zSelIndex]` is now 0, jump to `@8ab5` (leave); else **stay in `$70`**
    for another use (`item.asm:2194-2202`).
...
(`field_menu.asm:3765-3766`) followed by `CharSelectCursorPos` — four `(x,y)`
pairs `(8,40) (8,88) (8,136) (8,184)` (`field_menu.asm:3770-3774`). It then
...
(`menu_common.asm:1205-1222`) gives

> **`$4B` = the party slot, 0..3.**
...
- **Up / Down only.** Left/Right are read (`menu_common.asm:1357-1378`) but with
  `$53 = 1` and `NO_X_WRAP` set they can never move.
- Up/Down **wrap** (flags bit 0 clear).
- It reads **`z0a`** (`$0A`), the *repeating* button word, not `z08`. Repeat
...
- Empty slots are skipped by the task loop, not by `MoveCursor`: the task calls
...
(`field_menu.asm:2851-2874`). **Do not press left/right during magic targeting.**
State `$70` (item target) has no such handler.

### Can a KO'd character be selected? Yes.
...
### Does the game refuse a Potion on a dead character? Yes.
...
So: **Potion on a full-HP character is refused. Potion on a KO'd character is
refused. Fenix Down on a healthy character is refused** (it only passes through
the dead branch).
...
init (`menu_init_2.asm:506`) and never re-zeroed, so **`$B5 != 0` is a sticky
"at least one refusal happened since the menu opened" flag**, and `$B5 & $F0 !=
...
(the item refusal we care about) — so it is a menu-wide flag, not
item-specific. `PlayInvalidSfx` itself writes only APU I/O
...
writes the clamp back to `$0009,y` (`field_menu.asm:2977-2980`). That is the
game writing its own state, not the harness, but a fixture diffing HP across a
refused Potion should know it can move.
...
fixture needs to assert *exactly* what the boost bits are at a given moment, the
...
So the menu slot is the **battle order** slot from `$1850+c` bits 3-4, *not*
party order, and:
...
(`menu_ram.inc:168-187`. The `$6F` figure is confirmed directly in the source:
`DrawCharBlock2` writes `ldx $6f`, `field_menu.asm:4267`.)

**Recommendation for the fixture: read `zCharID` at `$69+slot` and compute
`$1600 + 37*charID` yourself, or read the pointer word at `$6D + 2*slot`.**
Do not assume slot == character.
...
authoritative numbers are the `$1600` block; reading the tilemap would mean
...
**directly, no fade**, and start `CharSelectCursorTask`.
...
cannot be picked for Skills at all** (`field_menu.asm:722-731`,
...
note the source's own comment, *"this is the only cursor that doesn't have an
initial position of (0,0)"*. One column ⇒ `$4B = $4E`, and the initial `$4B` is
**1**, already on Magic.
...
So **`$7A == $20` is a readable "this character can open Magic" gate.**

L/R in `$0A` cycle to the previous/next valid character
(`CheckShoulderBtns`, `field_menu.asm:1024-1088`) — note it *skips the whole
handler while the mosaic is running* (`field_menu.asm:1026-1029`), the only
place mosaic gates input.

Magic selected: `SkillsOption_01` (`field_menu.asm:1219-1228`) sets
`zMenuState = $1A` **directly, no fade**.
...
(`skills.asm:112-117`), positions `skills.asm:119-129`. Scrolling list ⇒

> **`$4B = $53*$50 + $4F = 2*absoluteRow + column`**, 0..53
> (`CalcLongListIndex`, `menu_common.asm:1230-1246`).

`$4A` is the page scroll (0..19), `$4E = $50 - $4A` the row within the page.

**Movement is `MoveListCursor`** (`item.asm:790-885`), not `MoveCursor`: left at
column 0 wraps to the previous row's last column and scrolls if needed
...
pressing A — that is exactly the gate `MenuState_1a` applies
(`field_menu.asm:2396-2402`).

**Trap — `$7E9D89` is mutated by drawing.** `DrawMagicListRow`
...
`GetListTextPos`, `item.asm:1168-1181`). So entries for pages you have not
scrolled to still hold their original spell id. Consequences:

- `$7E9D89[i] == $2D` does **not** by itself mean the character knows Cure.
- `$7E9D89[i] == $FF` on a drawn page **does** mean "not usable/not known".

The **authoritative** known-spell test is the learn array: `$1A6E + 54*actor +
spellId`, where `actor` is the byte at `$1600 + 37*charID + 0` — *not* the
...
draws MP costs and is the mode that `$FF`s partially-learned spells hardest
...
bit `$20` (Gold Hairpin) → half. So **`$7E9E09[i] == $20` already encodes
"knows it, can cast it outside battle, and has the MP"**.
...
`$7E9D89[$99]` — so `$99` is the readable "which spell was chosen".
...
- **L / R / Left / Right first** → if `MagicProp & $20` (can target all), save
  `$4E→$5F`, clear `z46` bits `$06`, `zMenuState = $3D`
  (`field_menu.asm:2852-2874`). Cure is a party-targetable spell in vanilla, so
  expect this to fire. **Avoid these buttons.**
...
`_c32c14` (`field_menu.asm:3043-3086`) — the spell-side twin of
`CheckCanUseItem`:
...
`zMenuState = $00`. Otherwise you stay in `$3B` and can cast again.
...
1. **Transitional states.** Every fade is `$00`/`$01` for 1 frame then `$02` for
   8. Poll for the *destination*, never assert "we are still in X"
   (`field_menu.asm:515-545`).
...
7. **Auto-repeat on held directions**: 8-frame delay then every 3 frames
   ...
   **not** auto-repeat in menus: `UpdateCtrlMenu` deliberately omits the
...
8. **Buttons are remappable.** `z06` passes the d-pad through but routes the 8
   ...
   (`ff6/src/field/init.asm:249`), so the default holds — but a fixture that
...
9. **Cursor memory.** If `$1D4E & $40` is set, the main-menu cursor
   ...
   (`field_menu.asm:1252-1255`, `menu_common.asm:2514-2527`) all resume where
   you left them instead of at their init positions. New game clears `$1D4E`
   (`ff6/src/field/init.asm:250`). **The item/magic *target* cursor is exempt**:
...
10. **A is checked before B in `$70`** (`item.asm:2149-2157`). Don't press both.
11. **`DecItemQty` decrements the *first* matching slot, not `zSelIndex`**
    (`equip.asm:2509-2522`). With duplicate stacks of the same item the slot
    that shrinks may not be the slot you selected — while `MenuState_70`'s
    "did it run out?" test reads `$1969[zSelIndex]`
...
14. No confirmation window exists anywhere on either path — every A press is
...
All reads. Nothing here is written by the harness.
...
| `$4B` | cursor index | **the load-bearing cell** — see per-screen table below |
...
Preconditions worth asserting *before* pressing A on the target, since each is a
guaranteed refusal (§3): Potion/Tonic needs `status1 & $C2 == 0` **and**
...
drive up/down (never left/right — see trap 4 is for $3B; here left/right are
  legitimate column moves) until $26==$1A and $4B == i
press A
...
Note the asymmetry: in `$1A` left/right are normal column movement
(`MoveListCursor`, `item.asm:815-880`); in `$3B` they are the all-targets
shortcut (`field_menu.asm:2852-2874`).

---

## 9. What I could not determine from the source

- **Whether Cure `$2D` actually has `MagicProp & $20`** (party-targetable), i.e.
  whether left/right in `$3B` really flips to `$3D` for Cure. `MagicProp` is
  `.import`ed into `skills.asm` (`skills.asm:29`) and lives outside
  `src/menu`; I did not chase the data table. Probe: in `$3B` with Cure
  selected, tap Left once and read `$26` — `$3D` means yes.
- **Whether `MagicProp+3 & $01` is set for Cure** (usable outside battle). Same
  table. The observable proxy is `$7E9E09[i] == $20`, which the game itself uses
  as the gate, so a fixture never needs the ROM value.
- **The exact `UpdateEquip` rules that set the HP/MP boost bits** at `+$0C`/`+$10`.
  Probe named in §4.
- **Whether inventory slot 255 is truly unreachable.** Probe named in §2.2.
- **Fail-before/pass-after**: none of this was executed. Every state number,
  cell address and button rule above is a *reading* of the assembly, not an
  observation of the running game. The first fixture built on it should log
  `$26` every frame across one full heal and one full cast and diff that trace
  against §8; that is the cheap way to falsify the whole document at once.
