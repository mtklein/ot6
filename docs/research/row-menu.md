...
- **First thing every frame:** `jsr _c36989` (`field_menu.asm:1786`). This is the
  commit to `$1850` — see §2.2. It runs *before* the button handling.
- **B or RIGHT** → cancel: `zWaitCounter = 6`, `zMenuScrollRate = +12`,
  ...
  in one frame, no fade (`field_menu.asm:550-557`). **RIGHT is a cancel here** —
  do not press it.
...
There is **no validity check** on A. Unlike the Skills/Equip/Relic path, which
runs `CheckSkillValid` and refuses wounded/petrified/zombie characters
...
- **A** with `zSelIndex == $4B` (*same slot*) → **row toggle**:
...
### 1.6 What `_c32e10` actually does
...
So the toggle is **`$75 + slot` bit `$20`**, plus a portrait animation. Nothing
...
`const.inc` `.enum CHAR`). The stride of `$1850` is 1 byte, **not** 37 — the
`$1600 + 37*c` block does not contain the row at all.
...
which is then re-supplied from the slot index. So the row bit survives verbatim
and the battle-order bits are rewritten from the current slot — which is exactly
how the same screen commits a party reorder.

`_c36989` is called from exactly one gameplay site — `field_menu.asm:1786`
(`MenuState_0f`). The other five call sites are all save/load bookkeeping
(`field_menu.asm:2020-2021`, `:2053-2054`, `:2069`, `:2741`). **If you never
return to `$0F` after the toggle, `$1850` is never written.** In practice
...
- `PushCharFlags` **before** (`field/menu.asm:305`) — `obj.asm:3650-3669` copies
  `$0867 & $E7 | ($1850 & $18)` into `$1850`, i.e. the object copy is
  authoritative for the row bit *going in*.
- `PopCharFlags` **after** (`field/menu.asm:328`) — `obj.asm:3630-3644` copies
  `$1850` straight into `$0867` for all 16 characters.

So the menu's `$1850` write propagates outward to `$0867`, and the next
`PushCharFlags` (before a battle, `field/battle.asm:66-91`) copies it back
unchanged. **`$1850 + c` bit `$20` is the durable, save-backed answer**
...
1. **`$1850 + charID`, bit `$20`.** Master, persists after the menu closes,
   independent of party slot. Get `charID` from `zCharID` at `$69 + slot`
   (`menu_ram.inc:168-173`) — do **not** assume slot == character
   (`menu_init.asm:42-51`).
2. **DP `$75 + slot`, bit `$20`**, while the menu is open. This is what
   `_c32e10` writes, so it changes one frame earlier than `$1850`. Useful to
   assert the toggle fired *before* asserting it committed.
...
`field-care-menu.md` §3 will **not** fire here, and a fixture must verify by
reading the bit, not by waiting for a refusal signal.

---

## 3. What the back row actually changes in this ROM
...
**The exemption rule, and it is the load-bearing fact:**

`ExecCmd` sets `$B2 = $FF` and **`$B3 = $FF`** at the top of *every* command
(`battle_main.asm:3131-3133`). So "ignore attacker row" starts **set** — the
default for every command in the game is *no row penalty*. Exactly one routine
can clear it: `_c2299f` / `_magicpunch`, the weapon-swing setup
...
So: `eor #$20` inverts the weapon's BACK_ROW bit before `trb`. Weapon **has**
BACK_ROW → `$B3.5` stays set → **no penalty**. Weapon **lacks** it → `$B3.5`
cleared → **damage halved**.

**Which commands reach `_c2299f` at all.** It has exactly two callers
...
touches bit `$20`**. Confirmed exempt, with the command's own path:
...
Magic does full damage to the back row. So does anything with "ignore defense" —
...
monster swinging a "ranged" attack still does half to a back-row character.
...
- The in-battle **Row command `$14`** (`battle_main.asm:4133-4140`) toggles
  ...
  battle. It costs a turn (it goes through `ExecSelfAttack`).

A fixture that asserts "Edgar is in the back row during battle" must therefore
read `$201F` too, or `$3AA1` will disagree with `$1850` through no fault of the
menu.

---

## 4. Terra / Locke / Edgar: who loses nothing?
...
the weapon-flags byte (`battle_main.asm:2670-2671`). Reading that file directly:
...
| | Fight from the back row | Their real damage command | Net |
|---|---|---|---|
| **Edgar** | halved (Mithril Blade has no `BACK_ROW`) | **Tools** — `Cmd_09` never reaches `_c2299f`, §3.2 — **unpenalised** | **loses nothing**. And note **Edgar cannot equip *any* back-row weapon before Illumina** — no `$41`-`$52` record lists char 4. His Fight is permanently halved in the back row; it just does not matter. |
| **Terra** | halved with Mithril Knife | **Magic** — unpenalised | **loses nothing while she casts.** If she ever Fights she pays, unless she is holding a `FLAIL` `$44` or `MORNING_STAR` `$46`, either of which makes her Fight free too. |
| **Locke** | halved with Dirk | **Steal** deals no damage at all (`Cmd_05` → `_c2298a`, `battle_main.asm:3406-3417`) — so his *only* damage is Fight | **loses real damage**, and is the one of the three who genuinely trades. He can buy the trade back with `FULL_MOON` `$45` / `BOOMERANG` `$47` / `HAWK_EYE` `$49`. |

All three gain **half physical damage taken** (§3.3) for that price.
...
determine which town each shop record is**; that mapping is in event/map data I
did not chase. Probe: read the shop index the game passes to the shop menu, or
grep the field event scripts for `shop` commands.
...
3. **RIGHT cancels in `$0F`.** `MenuState_0f` treats RIGHT exactly like B
   (`field_menu.asm:1787-1792`). Only UP/DOWN are safe.
4. **The cursor wraps.** `CharSelectCursorProp: cursor_prop {0,0}, {1,4},
   ...
   up/down wrap around all four slots. With `$53 = 1` left/right cannot move at
   all. `$4B` **is** the party slot 0-3.
5. **Empty slots are selectable in `$10` but not in `$0F`.**
   ...
   `$55 != 0` (`field_menu.asm:3749-3756`). But `MenuState_0f`'s A handler calls
   ...
   vanilla "move a character to an empty position"; it cannot produce a row
   toggle, because a toggle requires `$4B == zSelIndex` and `zSelIndex` was
...
6. **No KO gate.** Nothing on this path calls `CheckSkillValid`
   ...
   can be moved to the back row. This is not the Skills screen.
7. **It costs no turn and no resource.** The field row toggle is free. The
   *in-battle* Row command `$14` does cost a turn, and does not persist (§3.4).
8. **No auto-close.** `$0F` stays until you press B or RIGHT. You can toggle
   several characters in one visit.
...
Assert `before != after` and not a fixed value: whether the character starts in
the front row depends on the save, and asserting "ends up back row" would pass
vacuously if the toggle never ran. If a fixture wants an absolute end state, it
should read the bit, decide whether a toggle is needed, and assert the bit
afterwards *and* assert that a toggle was performed when one was needed.
...
- **Fail-before / pass-after: none of this was executed.** Every state number,
  ...
  observation of the running game. The cheap falsification is to log `$26` every
  frame across one full toggle and diff the trace against §7, and to log
  `$1850+charID` and `$75+slot` across the same window.
- **The exact frame on which `$1850` updates.** I argue it is the first frame
...
- **Which town owns which `shop_prop.dat` record** (§4.4) — that mapping is in
  map/event data I did not read.
