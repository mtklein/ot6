...
> **CORRECTION — 2026-07-30. The "Lines (at map time)" column in §1 is dead
> — not one row of it still resolves.** The caveat above is right in
> principle and useless in practice: drift runs **+24 lines** at the top of
> `battle_main.asm` to **+376** at the bottom, so every number in that
> column now points at unrelated code. Use the label names, which are what
> the caveat says are stable, and this table for today's lines:
>
> | label | §1 says | today |
> |---|---|---|
> | `CalcTargetDmg:` | 1809 | **1833** |
> | join `@0c1e` | 1876 | **1901** |
> | `CalcMaxDmg:` | 1891 | **1917** |
> | `CalcDmgMod:` | 1961 | **1987** |
> | `CalcSpeed:` | 1539 | **1563** |
> | `_c208c6:` | 1383 | **1408** |
> | `CheckHit:` | 5796 | **5869** |
> | `InitBattle:` | 6053 | **6126** |
> | `CalcMagicDmg:` / `CalcDmg:` | 7186 / 7217 | **7372 / 7404** |
> | `LoadMonsterProp:` | 7307 | **7495** |
> | `LoadRageProp:` | 7504 | **7692** |
> | `InitMonsters:` | 7672 | **7872** |
> | `ExecAttack:` | 8015 | **8215** |
> | `UpdateStatus:` | 11032 | **11236** |
> | `ApplyStatusMod:` | 11169 | **11373** |
> | `SetStatus_14:` / `_19:` | 11594 / 11618 | **11798 / 11822** |
> | `SetStatusTbl:` / `RemoveStatusTbl:` | 11661 / 11696 | **11865 / 11900** |
> | `CreateImmediateAction:` / `Cmd_29:` | 12881 / 13335 | **13097 / 13555** |
> | `DecCounters:` | 14839 | **15201** |
> | `_c25b06:` | 14939 | **15315** |
> | `UpdateMonsterGfxBuf:` (btlgfx) | 15169 | **15545** |
> | `MenuTextCmd_0b:` (btlgfx) | ~16048 | **16108** |
> | `UpdateMenuState_38:` (btlgfx) | 16767 | **16824** |
>
> **These are label lines, regenerated by grep — regenerate them the same
> way rather than trusting this block in six months.** The interior line
> *ranges* inside §1's Notes column (the `1844; 1846; 1850–1856` style
> breakdowns) have **not** been re-derived; treat them as offsets from the
> label, not as addresses.
>
> Four other things in this file no longer resolve:
>
> - **`ot6.asm:108`, `:114` and `:1060` are past EOF.**
>   `ff6/src/battle/ot6.asm` is a **43-line include manifest** now. The
>   `$ee`-is-border-fill fix moved to `ot6_icons.asm`; the two stale-RAM
>   notes are in `ot6_break.asm`.
> - **"a `Cmd_20` mid-battle monster reload runs no `InitBattle` clear
>   (`battle_main.asm:7519`)"** — `:7519` is a bare `iny2` inside
>   `LoadMonsterProp` and cannot be the evidence. The *claim* is
>   independently supported by `ot6_break.asm:43-45`, which says the reveal
>   masks must be rewritten unconditionally at every seed precisely
>   because a `Cmd_20` reload has no `InitBattle` clear.
> - **"OT6 has spent 55 [bytes] of the `battle_code` shim budget"** — there
>   are **37** `jsl Ot6*` sites in `battle_main.asm` alone, which is 148
>   bytes before counting inline edits.
> - **`tools/encode_text.py` does not exist**; it is
>   `ff6/tools/encode_text.py`, and gotcha 7.1 later in this same file uses
>   the correct `romtools.encode_text` name.
>
> §2's "OT6 allocation" line also predates `ot6_memory.inc`: `$3E9C/$3E9D,X`
> are not "spare (M2 BP)", they are `OT6_BP_CLASS` / `OT6_BOOST_REVEALED`,
> and `$3ECB-$3ED2` is `OT6_DIVINE_USED` plus scratch, not a display buffer.
> **`ot6_memory.inc` is the allocation map of record.**
