...
Gau's kit under the **Ochette model**: Veldt learning stays unlimited and the
battle menu offers an 8-slot loadout. It also covers the Dance-model MP rule
and the chance-verb boost tiers for Rage. The learn-many/equip-8 shape is
**settled by the owner** and is not a proposal.

**Canon boundary.** Vanilla Rage is the baseline and is kept almost entirely:
...
loop: he learns from hunting, and not from levels, items or story.
...
pool makes nearly everything in the game huntable eventually. That loop of
seeing a beast and going to get it is Gau's identity, and this design does not
touch it: no cap on learning, no curation of what the Veldt teaches, no change
to how Leap works (§6.3).

The problem in vanilla is the payoff surface: 200+ learned rages arrive in the
battle menu as an unsorted, unsearchable list (two columns, scroll position
capped at `$7c`, `btlgfx_main.asm:20238-20240`, so it was built for a list of
hundreds), and the one the player wants is 40 button-presses away while the ATB
runs. The Ochette model separates the two games:

- **The collection game** stays unlimited and happens on the Veldt, as a
  completionist's collection.
- **The preparation game** is new and happens in the field menu: the player
  chooses which eight rages to carry. Eight is enough to cover roles (a healer
  beast, one per element, a status special, a high-damage special) and few
  enough that each slot is a real decision. It is the argument about curating
  being part of a character's identity that `kits.md`'s organizing principles
  already make, at the slot count the owner named.
- **The battle game** is vanilla possession: pick one of the eight, pay once,
  and stay possessed until the fight ends. The loss of control is part of the
  verb, which is the same ruling Dance made (`kits.md`'s Mog sketch).
...
as long as Sabin's, and what differentiates Gau is that his eight are chosen
from hundreds, not the shape of the menu.

Gau already sits inside the part of the game that is playable today: the s2
...
`gen_sabin_gau` sits in the smoke set (`Makefile:200`). Natural-boot coverage
rides that savestate (§9).
...
The 8 loadout bytes must persist per save. Three candidate locations were
weighed. The decisive evidence is **how the Bushido word itself was added** and
**what the tracked checkpoints hold**.
...
`persistent_layout` string `ot6-codex-o8-v1`, declared by every real checkpoint
(`tools/tests/checkpoints/*/manifest.json`) and by every consuming segment
(`OT6_CHECKPOINT_LAYOUT:` markers), was defined at a point when the loadout word
already existed. The precedent is therefore exact: **a zero-sentinel addition
to the save-block scraps is not a layout change**, because no existing byte's
meaning changes and no old checkpoint can be misread.
...
- The checksum window covers them (`$1600-$1ffd`, `save.asm:750-771`), and the
  per-slot round-trip copies them (`save.asm:69/:97`), so persistence and
  validation cost nothing extra, the same as for the word.
- **All 11 tracked checkpoints, all 3 slots each, read `$1e1d-$1e28` all-zero**
  (payload scan of `tools/tests/checkpoints/*/`). The zero-sentinel migration
  is a measured property of those checkpoints rather than an assumption.
...
| Checkpoint cost | **9 real checkpoints** (11 tracked minus the two negative fixtures) regenerated through real Save UI drives, in dependency order, which means regenerating the whole chain of savestates at the multi-hour cost checkpoint-fixtures.md was written to avoid, plus editing every segment's `OT6_CHECKPOINT_LAYOUT` marker and manifest | none *claimed*; see the accuracy row | **zero** (measured zeros decode as AUTO, the semantics those saves already had) |
| Checksummed / save-slot semantics | designer's choice | **no**: codex pages sit outside the `$1600-$1ffd` checksum, and codex writes are immediate, so a loadout there would survive a reset-without-save, unlike every other menu decision in the game | **yes**, both, at no extra cost |
| Init guarantee | designer's choice | **none for the tail**: `Ot6CodexEnsure` wipes only `$300` bytes from `+$10` (`ot6_codex.asm:129-132` and siblings), so the tail is never initialized by any code path. Checkpoints happen to hold zeros there (measured), but that is the emulator's SRAM fill, not a contract; real-hardware SRAM is undefined, so the tail needs its own sub-signature to be safe | New Game zeroes the save block; sentinel = the uninitialized value (`ot6_kits.asm:18-22`'s argument, and §4.2's measurement) |
| Layout-string accuracy | accurate by construction (that is its whole cost) | **cannot stay accurate**: `ot6-codex-o8-v1` names the codex page layout *specifically*; claiming tail bytes changes the thing the string versions, so this option ends up as option (a) | stays accurate: the string's contract is "refuse a checkpoint the segment would misread" (`sram_checkpoint.py:22-33`, `checkpoint-fixtures.md`'s "Costs, named"), and no v1 checkpoint can be misread, because zeros are AUTO, which is the state those saves are in |
| Precedent | `checkpoint-fixtures.md` names the path but calls it "deliberate", for real schema breaks | none | **the Bushido word itself** |

### 4.4 The ruling

**Option (c): `OT6_RAGELOAD := $7e1e1f`, 8 bytes, byte = rage id + 1, `$00` =
unset, all-zero = AUTO. `persistent_layout` stays `ot6-codex-o8-v1`. Zero
checkpoints regenerated.** With `ot6_memory.inc` asserts pinning the range inside
`$1600..$1ffd`, exactly as the word's asserts do (`ot6_memory.inc:39-40`).

Two riders:

1. The scrap budget is now `$1e27-$1e3f` (25 bytes) plus `$1e70-$1e7f`, which
   is finite. When it runs out, option (a) is the right path; checkpoint-fixtures.md's
   regenerate-and-migrate discipline exists for that case, and spending 8 of 35
   bytes on a major kit feature is what the scrap is for.
2. `persistent_layout` versions the *schema*, and this ruling extends the
   ...
   ride option (c)'s bytes unchanged, at the cost of regenerating the 9
   checkpoints and no change to the design. Recommended: no bump, matching the
   word's own precedent; the segment-entry invariant contracts (`checkpoint-fixtures.md`,
...
makes it one *purchase*. `mp-economy.md`'s Dance row in "The verb survey" says
"one payment starts a whole-battle state — vanilla's can't-stop-dancing lock is
preserved, so the price is per battle, not per step", and one pricing rule
governs both possess-verbs. A per-rage formula (price by the special's spell
cost) was considered and rejected on three counts. It double-charges: the
trance's real price is the surrendered control, already paid, and the special
is only *rolled*, not chosen per turn. It works against the collection: the
rarest hunts would carry the highest prices, taxing the entries the collection
game exists to reward. And it needs a 255-row price surface where the flat rule
needs one number on every row (§2.2's display note). 8 sits in the top half
of Dance's own 4-10 range, because Gau's set of rages is broader than Mog's
eight dances, so Rage is priced just under that range's ceiling rather than
above it. 8 is also ~20% of Gau's join-era pool (base 10, `mp-economy.md`'s
"Early pools", plus the universal pool), which is a real commitment but never a
lockout, and it is cheap next to a 40 MP divine because the payment buys a
*gamble* rather than an apex ability. If Dance ever lands on a different number
inside 4-10, **Rage follows it**: the rule that possess-verbs share one flat
price outranks the 8 given here.
...
source, there is nothing for it to buy:
...
- **The only gamble is the return cadence**, meaning when Gau reappears
  ...
  the gate reads `$11e4` bit 0, whose field-side writer is unread. Reading it
  is the rider's prerequisite.

**Leap ships vanilla and the rider is deferred.** If the return-cadence roll
is located and playtest finds the wait frustrating, the rider should shorten
the absence (1-3 BP → sooner/next-battle-guaranteed return), which is
certainty in the thing Leap gambles on: the absence, not the capture.
Until then Leap joins Break and Doom in the boost-inert ledger (§10.5): the UI
accepts a spend that buys nothing, which is the known canon gap
(magicite-tube-six.md §13.4) rather than a new one. Reshaping Leap into an
in-battle Capture roll (the superseded `Leap→Capture ✦` sketch) is rejected,
because it would remove the leap-and-return loop the owner's "the collection
game IS Gau" framing protects.

---

## 7. Balance notes

- **The tuning now happens in Gau's own menu**: 8 slots against 200+
  candidates is the entire tuning surface, and it balances the trash/boss
  split on its own, since carrying a healer beast means carrying one fewer
  damage beast. M6's measurement is wear-time per rage across the s2 fixtures,
  the magicite-tube-six §10.3 discipline pointed at loadout slots instead of
  stones. Unmeasured.
- **3 BP plus the right special is the strongest purchase here**, and it is
  paid for three times over: 3 BP (a full bank, with Octopath's no-regen rule
  applying on the next turn), the 8 MP trance price, and the loss of control.
  A guaranteed Aqua Rake every turn at Baren Falls-era power is strong; the
  levers, in order, are the tier thresholds (§6's table), not the loadout
  width and not a flat price that differs from Dance's.
- **The confusion interaction adds depth at no cost**: a muddled unset-rager
  rolls a random rage *from the loadout* (§2.2), so preparation also
  constrains Gau's failure modes.
...
General rule: a hook that stops handing back to vanilla becomes a new hot
path. Measure its cost and re-run the whole suite rather than only the
feature's own test.
...
2. **The Dance number**: Rage's 8 follows whatever lands there, because one
   flat price for possess-verbs is the rule.
...
4. **Strict-reading layout bump** (§4.4 rider 2): if `persistent_layout` is to
   be bumped on principle, budget regenerating the 9 checkpoints and do it
   deliberately per `checkpoint-fixtures.md`'s "Costs, named"; the byte placement
...
6. **The Bushido configurator's geometry** (§8.0b): Skills → SwdTech draws its
   ...
   window. That is visible in `menu_swdtechpage.lua`'s own screenshot and is
   not caught by any assertion in the suite. Needs a layout pass of its own plus a
