...
`gen_sabin_gau` (`tools/tests/frontier_graph.py:190-197`), and `gen_sabin_gau`
...
(`tools/tests/anchors/*/manifest.json`) and by every consuming segment
...
| Layout-string accuracy | accurate by construction (that is its whole cost) | **cannot stay accurate**: `ot6-codex-o8-v1` names the codex page layout *specifically*; claiming tail bytes changes exactly the thing the string versions, so this option quietly converges on option (a) anyway | stays accurate: the string's contract is "refuse a checkpoint the segment would misread" (`sram_anchor.py:22-33`, `leg-fixtures.md`'s "Costs, named"), and no v1 checkpoint can be misread — zeros are AUTO, the state those saves are genuinely in |
...
1. The scrap budget is now `$1e27-$1e3f` (25 bytes) plus `$1e70-$1e7f` —
   finite. When it runs out, option (a) is the right path; leg-fixtures.md's
...
2. `persistent_layout` versions the *schema*, and this ruling extends the
   ...
   word's own precedent; the segment-entry invariant contracts (`leg-fixtures.md`,
