...
(Sabin scenario) chain carries him — the `gau_joined` savestate, generated by
`gen_sabin_gau` (`tools/tests/savestate_graph.py:190-197`), and `gen_sabin_gau`
sits in the smoke set (`Makefile:200`). Natural-boot coverage rides that
savestate (§9).
...
tracked checkpoints actually hold**.
...
(`OT6_CHECKPOINT_LAYOUT:` markers), was coined for a world that already contained
...
- **All 11 tracked checkpoints, all 3 slots each, read `$1e1d-$1e28` all-zero**
  ...
  story is not an argument — it is a property they demonstrably have.
...
| Checkpoint cost | **9 real checkpoints** (11 tracked minus the two negative fixtures) regenerated through real Save UI drives, in dependency order — effectively regenerating the whole chain of savestates, the multi-hour cost checkpoint-fixtures.md was written to stop paying, plus every segment's `OT6_CHECKPOINT_LAYOUT` marker and manifest edited | none *claimed* — but see the accuracy row | **zero** (measured zeros decode as AUTO — the exact semantics those saves had) |
...
1. The scrap budget is now `$1e27-$1e3f` (25 bytes) plus `$1e70-$1e7f` —
   finite. When it runs out, option (a) is the right path; checkpoint-fixtures.md's
...
2. `persistent_layout` versions the *schema*, and this ruling extends the
   ...
   ride option (c)'s bytes unchanged — the cost is only regenerating the 9
   checkpoints, not a different design. Recommended: no bump, matching the
...
of Dance's own 4-10 range — Rage's stable is broader than Mog's eight dances,
so it prices just under that range's ceiling, not above it — and it is
...
- **Mid-trance turns touch no boost machinery at all**: no fold, no
  ...
  (§10.8 — a real cost, and Cyan-adjacent: banking with a purpose).
...
is located and playtest finds the wait frustrating, the sensible rider shape is
...
- **The slot fight is Gau's own menu now**: 8 slots against 200+ candidates is
  ...
  wear-time per rage across the s2 fixtures — the magicite-tube-six §10.3
...
## 10. What the machinery cannot express — the full ledger
...
2. **Mid-trance rage switching.** The possession runs to battle end or death
   ...
   plus a re-pick UI that doesn't exist. Deliberately not wanted — the lock is the
   verb — but it is also not *available*, which is the half worth saying.
...
8. **Spending BP mid-trance.** A possessed Gau banks regen he cannot spend —
   no menu, no confirm moment. A real cost of the possession, priced into §6's
