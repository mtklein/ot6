...
1. **The stop line.** The area ends at
   `event_main.asm:78007-78009`: `load_map 0, {249,128}` + `airship_pos
   {249,127}`. Control returns on the WoB world map at **(249,128)**, one tile
   from the repaired, re-flying(?) Blackjack, party
   **TERRA·LOCKE·STRAGO·RELM**, with the full roster re-normalized and
   re-available except Shadow
   ...
   works immediately, is **UNVERIFIED**; it is v0.9's first probe. (`$007A` is
   never cleared in the area, and the fly-refusal script branches on
   `$007D=1` to `_cb1fef`, which is unread.)
2. **Two joins, one guest, and two Shadow departures.** Strago joins mid-fire
   (`char_party STRAGO,1` `:71794`, `$02E7/$02F7=1` `:71796-71797`, with no
   ...
   later with `remove_equip SHADOW` (`:73018`), which returns his gear to
   inventory. General Leo is player-controlled and solo for the massacre; he
   rides the WEDGE actor slot (`char_prop WEDGE, LEO` `:76352`,
   `char_party WEDGE,1` `:76354`).
3. **Sketch becomes usable inside battle 125, and the fight's scripted finish
   is a successful Sketch.** Relm is an active party member when the Ultros ③
   ...
   Sketch is the only player source of Tentacle. The scripted finish and the
   shipped bug depend on the same die roll: a missed Sketch is the bug's
   entry condition (§5).
4. **The area grants zero magicite.** No `give_genju` exists anywhere in
   `:69190-78110` (grepped). Kefka takes the drained espers and the player
   gets none. `magicite.md`'s "Bismark — Thamasa" acquisition proposal is
   contradicted by shipped state.
5. **The massacre is one indivisible auto-chain, the area's longest set
   piece.** From the mountain-top trigger 375 (15,17) (`_cbf2b5` `:74063`) the script
   ...
   long entrances, its own block is empty, and the town cannot be exited).
   ...
   triggers (`_cc0960` `:78041`, `battle 75` `:78048`, with no once-latch). No
...
6. **Vanilla already covers Ultros ③.** The mountain save point at 375
   ...
   (375 (2,45) → 371 (9,9), short-entrance table). The area's two save gaps
   are the **burning house** and the **massacre approach**. Plugging both
   costs 2 triggers + 2 NPC records, which is the entire remaining game-wide
...
7. **The relocation is two constants and one Lua pin.** Bank C4's segments
   ...
   `tools/tests/battle_runic.lua:68` (`MAGIC_PROP = 0x046AC0`). There is also
   a standing cost: every savestate regenerates; batch checkpoints survive (#9).
8. **The encounter shape repeats the Zozo inversion, and vanilla authored
   it.** Fire is the most effective element against Esper Mountain trash:
   Mandrake, Insecare, and Slurm are all weak to fire (§3.2). FlameEater then
   absorbs fire, so the element the mountain trains is the wrong one against
   the boss (bosses-wob §18 already describes this; the trash data matches).
   Ice and water are the counters: Balloons, FlameEater, and Aqua
   Breath's debut (via the authored add) all use them.
...
nowhere; empty trigger block). It follows the map-275 pattern and is
excluded.
...
Town on first entry (`$007D=1` branches): shops open. The weapon shop sells
...
pre-`$0099`). The inn charges 1500 GP until Strago is talked to
...
region, `name_menu RELM` `:70067`). No generator has needed this driving
idiom since the hello-world rename; the fixture must accept or confirm two
default names mid-scene. Sets `$008D=1` (`:69854`).

Sleeping at the inn (1 GP, `_cbd7ac` `:69540`, `$008D` branch `:69555` →
`_cbdcc7` `:70419`) plays the night scene: Shadow leaves the party
(`char_party SHADOW, 0` `:70456`), fire breaks out, `$0190=1 $008E=1`
(`:70634-70635`), and Shadow runs off after Interceptor and becomes
unavailable
(`$02F3=0` `:70653`). Control returns on map 343 at (12,21), retiled: map
343's init `_cbd41a` (`map_init_event.asm:362` → `:69289`) applies the
burning-house `mod_bg_tiles` blocks whenever `$008E && !$0090`, so town 343
...
ungated, so a world save mid-fire appears legal; **UNVERIFIED live**. The inn
refuses service during the fire ("FIRE!!", `:69493` → `:69566`).

### Segment 3 — the burning house and FlameEater (map 351)

Talk to Strago at the house door. This is an NPC event rather than a tile
trigger (`NPCProp::_343` record at (39,24) on switch `$0508`,
`npc_prop.asm:15217-15222` → `_cbde30` `:70668`). The scene ends with
Strago joining (`char_prop STRAGO` + `char_party STRAGO,1` + `$02E7=1
$02F7=1` + `max_hp/max_mp`, `:71790-71801`, with no `norm_lvl`; join level
...
- **Twelve wandering flame NPCs**, each `set_npc_movement RANDOM` with
  ...
  weak ice|water, sketch row SPECIAL/EXPLODER, so they self-destruct for
...
- **FlameEater** at (46,53) (`_cbe767` `:72095`): party order re-forced
  STRAGO·TERRA·LOCKE (`:72101`), then battle 79 (`:72124`). Formation 449,
  FlameEater `$116` L26 HP8400, absorbs fire, nulls bolt|poison|holy|earth,
  weak ice, plus the authored OT6 water add (`ot6_break.asm:472-479`;
  shields 7 · pierce, `ot6_hud.asm:1718-1719`; break coverage bosses-wob §18).
  `$0090=1` at `:72129`.

Two chests: Fire Rod (4,52) and Ice Rod (45,7). Rods break for a spell cast,
so the Ice Rod is a FlameEater counter picked up on the way in. After the
fight: the Relm/Interceptor rescue, Shadow's smoke-bomb
...
`remove_equip SHADOW` (`:73018`; his gear returns to inventory, and fixture
inventory assertions should expect it), Interceptor stays behind with Relm,
`$0092=1` (`:73302`). Town and world are then open. This is the last chance
to shop and world-save before the mountain.

### Segment 5 — Esper Mountain: 375 ⇄ 372/373/374 → 371

World entrance **0 (229,130) → 375 (55,31)** (short-entrance table; return
375 (55,32) → world). Whether the entrance is reachable before `$0098`, that
is, whether a party can sequence-break onto the mountain before the fire,
is **UNVERIFIED**. `$0098` is set with `$0091`, but nothing in
`event_main.asm` or `world` reads it as a gate, and the statue scene scripts
a STRAGO object it never checks for. This is a wedge risk; probe it first
(§7 hazard 8).
...
`:73358-73416`). Map 375's static tilemap no longer describes the live map
...
- Ultros choreography, then Relm is created and joins the active party
  ...
  $02F8=1`, `:73694-73701`, again with no `norm_lvl`);
- battle 125 (`:73702`). Formation 387, Ultros `$12e` L25 **HP22000**,
  ...
  surrender dialog → `kill_monsters ALL` (`:6315-6321`); see §5.
...
That is the last save before the massacre.
...
(dlg `$07FF`), the Terra-power scene, agreement to meet Leo (**`$0099=1`**
...
decode formation 445 = Guardian `$111` L71 HP50000. This is the vanilla
Leo-vs-Guardian fight; there is no once-latch, so it repeats on every
approach, and the XP goes to a character who leaves permanently, which
needs a fixture rule.
...
HP50001, whose AI is scripted only: espers cast Fire/Fire 2/Fire 3 at him,
then `battle_event $1a`, `end_battle` (`ai_script.asm:8047-8056`). Leo dies in
...
world-saveable.** This is the batch checkpoint (§2).
...
| # | where | what | evidence |
|---|---|---|---|
| S0 | world (232,150) | checkpoint K, the v0.7 terminal | `sealed-gate-route.md` §1 segment 7 |
...
(Veldt Cave, Floating-Continent-era maps, and an event-only interior), and
none is reachable from this area. The town has no save point in any variant,
and map 341 additionally cannot be exited.

The two gaps, measured against #10's principles:

- **The burning house.** The retry boundary for FlameEater is S2 (if the fire
  window allows leaving town) or the pre-inn world save. Either way
  a retry replays the fire-night scene, the flame gauntlet, or both. The
  gauntlet is short, but this is the area's only boss whose
  entry point has no save inside ~40 steps.
- **The massacre approach.** Battle 124 is a loseable solo fight about
  three scenes deep in an unskippable chain; a loss replays from S3 plus
  the entire mountain-top → Yura → town → Kefka sequence. This is the
  largest retry cost in the area, and it applies to the area's least
  conventional fight.

### 2.2 Proposed checkpoints (checkpoint-fixtures.md letters continue from K)
...
goodbye; one long interior stretch with no legal checkpoint inside,
splittable only by savestates at `house_entry` / `flameeater_won`), M→N
(world walk + mountain approach), N→O (statues + Ultros ③; short, and the
vanilla save point serves both ends), O→P (the massacre block: the atomic
chain of segment 6, with battle 124 as the risk point; savestate splits at
`massacre_start` / `kefka_duel_won`, never checkpoints). K→L and M→N are
short; L→M and O→P carry the work.

### 2.3 Entry contracts worth pinning

- L/M/N/O/P must assert the **availability vector** (`$02F3=0` from M
  onward; `$02E7/$02F7` from M; `$02E8/$02F8` from O). The area has many
  joins, and a stale checkpoint with the wrong roster passes size checks
  while breaking every scene's `party_chars`.
- M must assert Shadow's equipment is back in inventory (`remove_equip`,
  `:73018`), using the #21 count-assert pattern applied to gear.
...
1. **A save point in Thamasa town on map 343** (e.g. the square near
   ...
   This is the simplest option and the one with the most player value.
2. **A save point at the mountain top near 375 (15,17)**, the last
   controllable ground before the massacre chain. It cuts the battle-124
   retry from the whole chain to the chain from Yura on; the chain itself
   cannot be shortened, and the save only removes the mountain re-climb.
   1 trigger + 1 NPC.

Both together = 2 triggers + 2 NPCs, which is the whole remaining budget and
leaves none for the deferred v0.5/Opera backfill (~5 wanted per #10's
deferred list). The combined need (≈7) exceeds the budget (2), which matches
the dispatch's expectation. §6 is the relocation design that resolves it.

Zero mandatory spends is also an option. Vanilla
covers Ultros ③, the fire window may have S2, and the massacre gap is
mostly a fixture problem (savestates) that vanilla shipped this way.
Both placements score well against #10's published principles, and the
budget is the only reason not to make them, so the recommendation is to
raise the budget rather than skip the saves.
...
| **battle 124** | Kefka approach, Leo solo | 388 → KEFKA_VS_LEO `$173` HP5001 | **real and loseable**; ends only on Kefka's death (`ai_script.asm:7959-7964`); no OT6 row, so it falls to the generated break floor and will draw a gauge (decision 4) |
...
add shipped with #23 (`ot6_break.asm:472-479`).
`$173`/`$17a` (the massacre Kefkas) have no rows: the break floor will give
them a weapon-class weakness and the HUD will draw a formula gauge
(2+level/8; L1 → gauge 2) on fights that are script-shaped. Report/Report/decision 4.
...
- **Fire is the most effective element against mountain trash** (Mandrake,
  Insecare, Slurm), and FlameEater then absorbs it. Vanilla arranged this
  without any OT6 change. Terra's fire lean works on trash; the boss forces
  a switch to ice or water (Ice Rod chest, Aqua Breath, the shipped water add).
- **Enemies with no weakness** carry the other axis: Abolisher, Adamanchyt,
  Baskervor, and Chimera have empty weak rows and use the generated break
  floor (weapon classes only). The survey pass should decide whether any
  gets an authored element row, or whether class-only is intended
  for an area whose kit additions (rods = bludgeon, brush = special)
  already need weapon-class relevance.
- Levels: island L20-22, mountain L23-24, bosses L25-26, against a party
  arriving from v0.7 at roughly L16-19 (**unestablished**; v0.7 has not been
  generated, and wob-route measured L15-16 at the v0.6 tail). That gap is the
  area's balance question, since vanilla tuned Thamasa for parties around
  L22 and up. The XP available on the way (two scripted Balloon gauntlets,
  and a repeatable Guardian only Leo can farm) does not close it. Measure at
  authoring, per the M6 discipline.
...
- **Strago (Blue Mage/Scholar, `kits.md`'s "Curated kits").** Joins in segment 3
  ...
  Lore-by-observation, and Analyze itself (which depends on that same M4 kit
  work) are all open. Aqua Breath is free at join, Lores are MP-priced
  (`mp-economy.md`, "The verb survey"). Base MP 13 (`mp-economy.md`, "Early
  pools") under the now-universal pool (#32). This is the area's largest kit
  build.
- **Relm (Pictomancer, `kits.md`'s "Curated kits").** Joins in segment 5 inside
  ...
  TBD". v0.8 must at minimum decide the 8-slot outline of her kit even
  ...
  also deferred there ("priced when her kit lands"). Base MP 18.
  Boost canon: §5.5.
- **Shadow, the #31 entry debt.** He is party-active only from checkpoint K to
  the inn night (segments 1-2: one town, no mandatory fights, then absent until the
  Floating Continent). The v0.7 issue moved his kit here. The scope
  question is whether a kit reachable for about 20 controllable minutes
  with no required battles justifies the build now, or whether the debt
  moves again to v0.9, where he is forced party on the FC approach and
  the kit is needed. Throw is free by ruling (item is the price,
  `mp-economy.md`, "The verb survey"); Assassinate is built dormant since v0.4.
  Report/decision 3.
- **General Leo.** A real, loseable solo fight on a guest. Shock is free
  (`mp-economy.md`'s standing guest-verb ruling covers him; no kit
  table needed). Presentation is unsettled: battle 124's Kefka has
  no authored OT6 row, and Leo has no BP/boost tutorialization. Whether the
  gauge and boost HUD behave with a WEDGE-actor solo party is unknown. Probe;
  Report/decision 4.
- **Magicite: zero new stones** (headline 4). The area's M5 obligation is
  ...
  #31's commitment), v0.8 has no esper work, the first area for which that
  is true. If v0.7 re-scoped, the debt compounds here. Either way v0.8 adds
  no stones.
- **Equipment flow to assert in fixtures:** Shadow's gear returns at segment 4
  ...
  so any fixture asserting exact bench levels across the area boundary will
  see them move.
...
The owner's decision is shipped: the Sketch bug ships unfixed and
...
Battle 125 is the first live Sketch, and the fight is built around it. Relm
...
finish rewards Sketch: `MonsterSketch[302]` = TENTACLE,
...
is battle 125 plus optional mountain and world trash on the walk out.
From the stop line onward she is a free-pick party member and the exposure
is every battle she is in.

### 5.2 The mechanics, read (what "miss" means here)

`TargetEffect_55` (`battle_main.asm:9673-9700`) exits without fixing `$b7`
(the byte left invalid) in three cases: target is a character (`cpy #$08`),
target flagged unsketchable (`$3c80,y` bit `$20`), or the
`CheckSketchHit` roll fails. `CheckSketchHit` (`battle_main.asm:9065-9083`)
is a level ratio of attacker level to target level (Coronet scales it);
when the attacker's level is not clearly above the target's, a random roll
decides. The direction of the division was **read but not traced**, and the
operational consequence is the same either way: at the levels here (Relm
~L17-24 vs Ultros L25) a Sketch miss is a live outcome, plausibly ~20-30%
per cast, and every miss takes the `$b7=$ff` path into `AnimType_2f`'s
unguarded index (`vanilla-destructive-bugs.md`: severity is
data-dependent, and can be a corrupt sprite, spawned inventory, or a
bank-`$7e` sweep that reaches the save block and battle inventory). Ultros ③
is sketchable (flag bit clear; his sketch row is required for the scripted
finish).

### 5.3 What the release notes must say
...
sentences rather than a link:

1. **What:** Relm's Sketch carries FF6 1.0's best-known bug, deliberately.
...
2. **When:** misses happen against enemies of higher level than Relm and
   against vanished or invisible targets. Her debut fight (the octopus on the
   Esper Mountain) is above her level, so it carries a high miss chance, and
   sketching him is still the intended
   finish.
3. **Defense:** save before experimenting. The world map saves anywhere,
   and the mountain save point is just outside the statue room. [If
   Report/decision 1 ships the town save point, name it here too.]
4. **Why:** vanilla's destructive bugs normally get fixed (the save-slot
   checksum bug was, v0.6/#18). This one is a deliberate exception: the 1994
   original's behavior is kept by owner decision. It is a house rule, not a
   fix the project ran out of time for.

### 5.4 The fixture hazard, and the rule it needs

The dispatch asked whether a gen or playtest drive can trip the bug
accidentally. It can, with meaningful probability, and at the point where it
does the most damage:

- The obvious way to script battle 125 is the way the game teaches, by
  driving Sketch. Every driven Sketch at the levels here risks the ~1-in-4
  miss, and a miss during a savestate-generation run can sweep bank `$7e`
  (the save block and battle inventory) right before the harness writes the
  batch checkpoint (O `ultros-won`) that every downstream segment will trust.
  A corrupted but checksum-valid tracked checkpoint is the worst failure the
  checkpoint-fixtures design can produce, and the invariant contract is the
  only check against it.
...
1. **Savestate-generating scripts never issue Sketch.** Battle 125's generator ends
   the fight on HP (22000 HP is slow but deterministic). If the Sketch
   finish is wanted for route fidelity, use it only after a probe confirms the
   attacker-level ≥ target-level auto-hit condition holds for the generated
   party. At the levels expected here it will not hold, so use the HP finish.
2. **The Sketch scene gets one dedicated, isolated probe fixture**:
   savestate-in, savestate-out, feeding no checkpoint. It drives Sketch,
   observes both outcomes (Tentacle finish on hit; on a forced miss,
   watches `($76),3` / `$b7` per the research doc's settle-list) and
   documents the shipped behavior. That probe also supplies the release-notes
   evidence and settles the Sketch row of
   `vanilla-destructive-bugs.md`'s "Reported, unverified" table, which is
   worth doing even though no fix will follow.
3. **Checkpoint O's entry contract** (and P's) should include an inventory
   ...
   visibly instead of passing into the baseline unnoticed.

### 5.5 Reconciling the boost canon with ships-unfixed

The chance-verb canon (`kits.md`'s "chance-verb family", ROADMAP design
canon) slates
Sketch for boost-certainty: BP buys sketch selection, meaning the hit roll and
...
escape on the failure path. The two intersect at one branch, and in the safe
direction: boost-certainty forces
`CheckSketchHit` to succeed, and the success path is what writes a valid
`$b7` (`:9691`), so a boosted Sketch cannot reach the bug through
the miss exit. At 0 BP the verb is vanilla byte for byte, which canon
requires (Steal's shipped precedent), so the bug remains reachable
...
- Leave the invalid-target exits alone: a character-targeted or
  unsketchable-target Sketch is not a chance roll, and boost must not
  validate it. Those exits stay vanilla and stay bug-reachable, which is the
  shipped rule; the release notes' "misses" language covers it.

No conflict exists between the canon and the ruling; `kits.md`'s Relm entry
("bug preserved ✦") already records this.
...
- The bank's segments are laid sequentially by the linker; none of the
  chain has a fixed offset, and only `ending_gfx_1` is pinned, at `$ba00`
...
### 6.2 The design: grow in place rather than relocate

Because the chain is sequential and symbol-addressed, the relocation is two
constant changes:
...
reference automatically: the trigger/NPC pointer tables are `ptr_tbl` label
...
splices in `battle_main.asm`/`shop.asm`). Sixteen new trigger slots and
sixteen new NPC records cover v0.8
(≤2), the Opera backfill (~5), and the rest of the WoB with margin, and
spend only `$E0` of the `$1540` headroom.
...
1. **`tools/tests/battle_runic.lua:68`** pins `MAGIC_PROP = 0x046AC0` (with
   ...
   next growth needs no change.
2. **Every savestate is invalidated.** This is a ROM change like any other
   (`checkpoint-fixtures.md`, "The problem"); the full savestate chain regenerates
   once. Batch checkpoints survive (#9's measured property), which is
   why the checkpoint conversion should come before this change,
   and why the change should land in v0.8's existing ROM
   window: kit tables, encounter rows, and any new save points are ROM
   changes too, so one window means one regeneration.
3. **The BPS patch grows**: every byte from `C41A10` to `C4A4BF` shifts, so
   the patch carries a ~35 KB shifted region it did not before. This is
   cosmetic, but expect it in the release diff.
...
5. Not broken: the sparkle-append rule
   (`save-points-vector.md` §1 — append records at the end of a map's NPC
   block, never insert) is unaffected, and per-map NPC indices do not move.
...
being blocked on segment layout. If the dispatcher prefers deferring, the
fallback also works: v0.8 fits in the existing 2 slots (§2.4), and the
growth waits for the backfill. That fallback regenerates the whole savestate
chain a second time and saves no work.
...
1. **The massacre chain is atomic and loseable in the middle.** From 375
   ...
   windows. Players: the retry is S3 plus the whole chain; the §2.4
   mountain-top save point is the mitigation.
2. **Sketch in harness drives** (§5.4). One rule, that savestate-generating
   scripts never issue Sketch, removes the whole class. Adopt it before the
   first Relm fixture exists.
...
4. **Three mod_bg_tiles contexts**: town 343 during the fire (init
   ...
   the wrong state will produce wrong results, per the v0.6 §11 correction
   class. Run a live census at authoring.
...
6. **The `$0099→$009C` town lockdown**: any fixture assuming towns can be
   exited breaks on 341. The segment-6 exit-row triggers also start a
   repeatable Guardian fight, so a drive that wanders into an exit row
   mid-window runs a 50000-HP theater fight. Leo banks the XP and then
   leaves permanently, so the fight costs time and adds RNG with no benefit.
7. **Join levels are un-normalized at join** (Strago `:71790-71801`, Relm
   `:73694-73701`, with no `norm_lvl`; the area tail normalizes the bench,
...
8. **Sequence-break wedge risk at the mountain**: the statue scene scripts a
   ...
   gates the world entrance (229,130) before the fire (`$0098` is written but
   never read as a gate; grepped). If a pre-Strago party can walk in, the
   scene will probably hang. Vanilla may prevent it by geometry
   (**UNVERIFIED**). Run one probe at the entry point. It is not a route risk
...
1. **Battle 125's `battle_event $16`**: what happens under 10240
   HP (Relm dialog, a sketch tutorial scene, or something else). Probe
   `formationWords()` and ride the event.
2. **Can battle 124 be lost, and what does losing look like** (game over vs
   scripted continue)? Decides how strict the O→P fixture must be and
   what the release notes say about the Leo fight.
3. **Leo's level and kit surface** in battle 124 under OT6 (BP pips, whether
   boost input is live, whether a gauge is drawn on an unauthored `$173`).
   One savestate probe.
4. **Does the fire window allow leaving town** (S2)? One drive:
...
10. **Where does `battle_event $17` (Leo's fall) leave battle state?** This
    is the monster-dead-flag idiom question for the one real fight in the
    massacre.
...
| encounter decode chain | `break-coverage-vector.md` §1 method; world terrain OR `field/battle.asm:97-147` |
