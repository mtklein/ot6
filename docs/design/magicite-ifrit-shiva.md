...
| Boost on non-folding actions | ✅ | ×2/×4/×8 on base damage; exempted commands are fight `$00`, capture `$06`, bushido `$07`, steal `$05`, slot `$0f`, rage `$10` — **summons (`$19`) are not exempt** | `Ot6BoostDmg`, `ot6_boostdmg.asm` |
| Ability MP cost | ✅ live | Magic prices on the vanilla baseline (`GetMPCost`); `Ot6AbilityCostTbl` only keys blitz/bushido/tool ids. Since #219 a *boosted* cast is the exception: a spell in no tier family (and an esper's summon) pays `min(99, base × 2.5^boost)` through the same list byte `Ot6FoldPrices` already moves. The gate row above is also the price rule: a verb's price escalates exactly when `Ot6BoostDmg` multiplies it, so the six exempted commands are flat | `ot6_boost.asm` (`Ot6AbilityCost` header); mp-economy.md "Who pays it" |
