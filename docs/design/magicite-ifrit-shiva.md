...
| Ability MP cost | ✅ live | Magic prices on the vanilla baseline (`GetMPCost`); `Ot6AbilityCostTbl` only keys blitz/bushido/tool ids. Since #219 a *boosted* cast is the exception: a spell in no tier family (and an esper's summon) pays `min(99, base × 2.5^boost)` through the same list byte `Ot6FoldPrices` already moves | `ot6_boost.asm:352-400` (`Ot6AbilityCost` header); mp-economy.md "Where it lands" |
