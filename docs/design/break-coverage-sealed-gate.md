...
| Ing | `$048` | 2 | `OT6_PIERCE` | The swarm body — 32.6 % of all bodies, ×3 at 37.5 % on two maps. Pin the walking corpse; Edgar's AutoCrossbow (whole side, chips per hit) is the designed swarm answer — the Pipsqueak precedent (`break-coverage-vector.md` §8.1). Pearl and water back it. |
...
- **Failure demonstration (the tool's own `--drop-class` arm, built for
  exactly this):** `--area sealed-gate --drop-class bludg` fails loudly
