...
replay from power-on. A single wrong guess costs a full serial replay to
discover, which is what makes guessing expensive and iteration slow.

The pain compounds: the later routes are longer than anything minted so far.
...
- **`mint_anchor`** (`Makefile`) mints a state by cold-loading a tracked in-game
  ...
  savestate. The `post-opera-v1` anchor is the worked example.
- **Battery saves survive ROM changes.** The post-Opera anchor cold-loads
...
- **`make -jN frontier` already parallelises** where the graph allows — the three
  ...
  the Vargas rung. Runner isolation makes concurrent mints safe.
...
experiences, and an anchor is only producible where the game's own save routine
can run.
...
- **Every leg asserts its entry invariants before doing anything** — story
  ...
  anchor must fail loudly, naming what differed. This is mandatory per leg.
...
- **Tactical doorsteps cannot be battery saves.** "One A-press from battle 72" is
  ...
  anchor. Acceptable while the drive is short.
...
3. Cut anchors at the save-point boundaries, working **backwards**
...
- Should anchors be regenerated automatically when a leg's exit contract changes,
  or always deliberately? Automatic regeneration risks laundering a regression
  into the baseline.
