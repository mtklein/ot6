...
| Assassinate (Shadow's divine): his hit that breaks a non-boss kills it on that hit, or a hit on one already Broken; once per battle, never a boss | `ff6/src/battle/ot6_divine.asm` `Ot6AssassinateGate` (x attacker, y target: Shadow, latch `$3ECB` clear, monster, `OT6_BROKEN_TICKS` nonzero, `$3AA1.2` clear -> `$3DD4` Death + latch), reached from `ot6_break.asm` `Ot6HitJoin` after both chips on every landed hit, and from the ChooseTarget seam `Ot6Assassinate` | HANDLED | `battle_assassinate` (suite, camp_escaped): the ledger orders the ROM's writes within one action -- break store (cpu x = Shadow) -> in-proc Death mark -> ApplyDmg HP write -> UpdateStatus Death -- then once-per-battle with hpDrops as the loud control, and a boss-bitted body broken with no mark and no latch spent | any random with Shadow aboard; the divine used to be unreachable in early play because the doubled breaking hit killed every body the pool dealt before the seam could read it (#239) | #239 |
