...
|---|---|---|---|---|---|
| Battle type selection (the roll) | `ff6/src/battle/battle_main.asm:7799-7821` `ChooseBattleType` — masks `$2F48`, rolls `RandBitWithRate`, `stx $201f` (:7820) | UNHANDLED | no harness read of `$201F` anywhere in `lib/` or `gen_*` (0 hits); only `battle_shadowstays.lua:101,168` and `probe_shadow_leaves_wedge.lua:77` read it, for the Shadow-leaves roll | every random encounter on every walked map, world and field | **#185** (the symptom) |
| Normal arrangement (`$201F=0`) | `battle_main.asm:7864` `InitBattleType_00` | ASSUMED (the only shape the driver plans for) | `lib/ot6.lua:3654` `if mons ~= 0 then return {"right"}`, `:3658` `if chars == 0 then return {"right"}`, `:3736` `if mons == 0 then return {"left"}` | everywhere | **#185** |
| **Back attack** (`$201F=1`) | `battle_main.asm:7900-7909` `InitBattleType_01`: toggles every `$3AA1.5` row bit, `lda #$20 / tsb $b1` (:7907-7908). Enable bit = `$2F48` bit 5 (`battle-ram.txt:567`) | **UNHANDLED** | nothing reads `$201F` or `$b1` bit 5. `lib/ot6_field.lua:119` names "`$b1` … bit 5 back attack/pincer" in a comment but `:159` only masks `CANT_RUN=0x02` and `$2f4b` bit 0 | any non-scripted encounter: Narshe mines, Mt Kolts, Lete, both scenario legs, Zozo, Magitek Factory, esper tubes, minecart, Sealed Gate cave, Thamasa/Esper Mtn, FC | **#185** |
| **Pincer** (`$201F=2`) | `battle_main.asm:7889-7896` `InitBattleType_02` — forces every character front row, then `tsb $b1 #$20`. Also sets `$b1` bit 1 via `UpdateMonsterGfxBuf` when monsters live on both sides | PARTIAL | live: only as a *flee refusal* — `lib/ot6_field.lua:148,159` read `$b1` bit 1 and hand the fight to the tactical driver. Static: `tools/audit_encounters.py:96-98` decodes the permission bit per formation. The *target cursor* still hard-codes LEFT/RIGHT. One generator steers it correctly: `gen_thamasa_fire.lua:650,655` reads `$7ACE` | Thamasa ambush (`gen_thamasa_fire`), `gen_mrf_chute:269`, Zozo clock shaft (`gen_zozo3_clock:41`), any pool `audit_encounters` flags | **#185**; #150 (closed), #82 (closed) |
| **Side attack** (`$201F=3`) | `battle_main.asm:7865` (shares `InitBattleType_00`), enable bit `$2F48` bit 7; disabled when fewer than 3 allies alive (`:7811-7818`). Raises run difficulty per monster (`:15645-15646`) | **UNHANDLED** | no read of `$201F`; `audit_encounters.py` decodes **only** the pincer bit (`:98` `(w ^ 0x00F0) & 0x40`), never `$20` (back) or `$80` (side) | any 3-4 member party in a random; the whole post-reunion route | none |
...
|---|---|---|---|---|
| **Selected target group `$7ACE`** — `0` = monsters-left, `1` = characters-left, `2` = monsters-right, `3` = characters-right | `ff6/notes/battle-ram.txt:1698-1702`; driven by `btlgfx_main.asm:16905-16926` (RIGHT `inc2`, LEFT `dec2`, both gated on `w7e7ace & $02`) | **UNHANDLED in lib** | `lib/ot6.lua:3654,3658,3736` steer by hard-coded LEFT/RIGHT. The correct read exists in the tree: `gen_thamasa_fire.lua:650,655` — `if ace % 2 == 0 then return (ace == 0) and "right" or "left" end` — and in `probe_thamlab_revive.lua:151,163`, `probe_thamlab_deadboard.lua:782` | **This is the exact cell that makes #185 a 9000-frame hang instead of a two-line branch.** In a back attack the party is drawn on the right, so the monster group index is `2`, and the driver's unconditional `"left"` at `:3736` walks *away* from the monsters forever until the 24-pulse give-up confirms on whoever is lit |
...
| `$7B7D` chars-lit / `$7B7E` monsters-lit / `$7B7F` all-latch | `battle-ram.txt:1777-1779` | HANDLED | `lib/ot6.lua:2086-2091`, used at `:3652-3745` and in the park signature `:3400-3404` | correct, but it says *what is lit*, not *which direction reaches the other side* |
...
1. **Back attack `$201F=1` / `$b1` bit 5 — UNHANDLED.** The fix is `$7ACE` (`battle-ram.txt:1698-1702`), already written correctly at `gen_thamasa_fire.lua:650-655` and never promoted into `lib/ot6.lua:3654,3658,3736`. Issue **#185**.
2. **Side attack `$201F=3` — UNHANDLED.** Same cursor geometry, plus 3× run difficulty. No issue.
3. **Pincer target steering — ASSUMED** (the flee half is handled; the cursor half is not). Issue #185 covers it only by implication.
