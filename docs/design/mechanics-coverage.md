...
| Formation arrangement-permission flags `$2F48 spbn----` | `battle_main.asm:8216-8220` `LoadBattleProp`: `BattleProp,x eor #$00f0 -> $2f48`; bits documented `battle-ram.txt:565-567`. Data: `ff6/src/battle/battle_prop.dat` word `[f*4]` | UNHANDLED (live) / PARTIAL (static, pincer bit only) | 0 live reads of `$2F48`; `tools/audit_encounters.py:97-99` | every formation on the route | none for back/side |
...
| 5 | Imp | PARTIAL (detect only) | field cure `ot6_field.lua:1755` (Remedy); battle `gen_sabin_train.lua:993-997` detects Imp'd Sabin and declares the attempt **LOST** — no recovery | Phantom Train, Imp Song rows | **#110 (closed) — "Berserk or Imp on Sabin ends the Phantom Train break, with no cure in the scenario"** |
...
| 7 | Sleep | UNHANDLED | no read (`gen_sabin_train.lua:101` names it in a comment as a gap that "cost a diagnosis") | Lullaby rows, Phantom Train | none |
...
| 4 | Stop | **UNHANDLED** | no read; a Stopped actor simply never gets a turn and the driver's `menuStreak`/`parkN` watchdogs (`:4052-4078`) cannot tell that from a stall | Sealed Gate, FC, Number 024/128 | none |
...
| Tools | `$30` select, **`$2E` open**, `$2F` close | `:20613`, `:13182`, `:12802` | ASSUMED — only `$30` is known; `$2E`/`$2F` hit the unknown guard | `lib/ot6.lua:2422`; contrast the Throw family where all three are whitelisted | Edgar, every fight | #115 (closed) |
...
| Field random-encounter pool + fleeability | `map_prop.dat +5` bit 7, `sub_battle_group.dat`, `rand_battle_group.dat`, `battle_prop.dat` | HANDLED (static) | `tools/audit_encounters.py` — but **pincer bit only** (`:97-99`) | every field map | #82 (closed) |
