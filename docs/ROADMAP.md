...
**v0.16 — Clean Room** (shipped 2026-08-26): a patch byte-identical to
v0.15's, shipped deliberately. The release exercises the gate end to end
from an empty build directory — every fixture regenerated by replaying the
game, every check re-run, the whole run timed — after the v0.15 ledger's
process fixes (the gate checks results it already has instead of rerunning
them, and generation parallelism is ninja's own at nice priority).
