...
Promoted from rc1 unchanged after the project's first full human
playtest reached deeper than any prior build — combat reading tense in
the good way the tuning bands aimed for. Known gaps are listed below on
purpose — gaps and todos are nothing to be ashamed of; silent ones are.
Several are already fixed on main and ship with v0.7.

## How to play

Apply `ot6-v0.6.bps` to a Final Fantasy III (USA) 1.0 ROM with SHA-1
...
## Known gaps in this release, on purpose

*(Status at tag time: the cross-list menu-display bug — present since
v0.1 — plus boost-tiered Slot are already fixed/built on main for v0.7;
the in-battle MP wallet display, clockwork HUD timing, and Dance's MP
cost are in progress there.)*
