...
; Blitz mode (w7e6168 = 1) passes the MP grey through untouched.  Only ever
...
; reason: BESTOW cannot do anything at 0 BP -- there is no pip to hand over, and
; Ot6Bestow's first refusal is exactly that test -- so the row greys, and the
; player learns it before spending the turn rather than after.  This proc keeps
; its Bushido name because the shape is Bushido's ("a second grey reason, on top
; of the MP one, for a row the bank cannot reach"); the two arms share the
...
@afford:
        ...
        plx                     ; restore caller X
        bra     @pass
; #55: thief mode -- only BESTOW has a BP precondition, and it is "hold one".
@thief: lda     $4005,y         ; this row's id (column 1; Qty at +1, Flags +2)
        ...
        pla                     ; 0 bp: nothing to give -- grey it
