...
; ------------------------------------------------------------------------------
; ShopProp is the vanilla blob plus one named byte override.
;
...
; The ghost merchant in car B (shop 85) is the scenario's last shop; vanilla
; left this record one slot short (byte 8 is $FF).
;
; Fire Skean and not Bolt Edge.  Throwing $ab resolves to attack $51, element
; fire, and $ad to attack $53, element bolt (ThrowToolsItemTbl /
; ThrowToolsOffsetTbl, battle_main.asm:6648-6655; elements from
; magic_prop_en.dat +$01), and GhostTrain $106 is weak to fire|bolt|holy
; (monster_prop.dat +25 = $25).  Fire additionally covers Specter $156, which
; is fire|holy weak, so one item covers both monsters in the scenario.
GHOST_TRAIN_SHOP     = 85
...
; catch a wrong offset that happens to preserve the total.
